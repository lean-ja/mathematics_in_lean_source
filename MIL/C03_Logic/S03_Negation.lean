...
この例では *局所的な定義* をローカルコンテキストに追加する ``let`` タクティクを導入しています． ``let`` コマンドの後ろにカーソルを置くと，ゴールウィンドウに ``f : ℝ → ℝ := fun x ↦ 0`` という定義が追加されていることがわかるでしょう．Leanは必要なときに ``f`` の定義を展開します．特に ``f 1 ≤ f 0`` を ``le_refl`` で証明するとき，Leanは ``f 1`` と ``f 0`` を ``0`` に展開します．
...
OMIT. -/
/- TEXT:
1つ目と2つ目，そして4つ目はこれまでに見てきた方法でそのまま証明することができます．ぜひ試してみてください．しかし，3つ目はより難しいです．なぜなら「存在しないと仮定すると矛盾する」という事実から何かが存在すると結論付けなければならないからです．これは *古典的な* 数学的推論の一例です．背理法を用いて3つ目の含意を次のように証明することができます．
TEXT. -/
-- QUOTE:
example (h : ¬∀ x, P x) : ∃ x, ¬P x := by
  by_contra h'
