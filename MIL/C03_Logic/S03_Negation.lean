...
OMIT. -/
/- TEXT:
記号 ``¬`` は否定を表すもので， ``¬ x < y`` は ``x`` が ``y`` より小さくないことを表し， ``¬ x = y`` （または ``x ≠ y`` でも同じ意味になります）は ``x`` が ``y`` と等しくないことを表し， ``¬ ∃ z, x < z ∧ z < y`` は ``x`` と ``y`` の間に狭義に ``z`` が存在しないことを表します．Leanでは， ``¬ A`` という表記は ``A → False`` を省略したもので， ``A`` から矛盾が導かれると考えることができます．実用的に言えば，読者はもうすでに否定を扱う方法を知っているとも言えます: ``¬ A`` を証明するにあたっては ``h : A`` を導入してから ``False`` を示せば良く，また ``h : ¬ A`` と ``h' : A`` があれば， ``h'`` に ``h`` を適用することで ``False`` を得ることができます．
...
OMIT. -/
/- TEXT:
ゴールがそのコンテキストに置いて線形な等式と不等式から導かれるとき， ``linarith`` を使うのがしばしば便利であることを覚えておきましょう．
...
OMIT. -/
/- TEXT:
今までの証明の多くの中で暗黙のうちに用いてきた事実として，次の2つがあります．もし任意の性質 ``P`` について，「性質 ``P`` を満たすものが何もない」ということは，「すべてのものが性質 ``P`` を持たない」ということと同義であり，また「すべてのものが性質 ``P`` を持たないわけではない」ということは，「ある特定の何かが性質 ``P`` を持たない」ということと同義です．言い換えるならば，以下の4つの含意がすべて成立するということです（ただし，そのうちの1つはこれまでに説明した内容では証明できません）:
TEXT. -/
-- BOTH:
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
本節の最後に，矛盾からはなんでも導かれるという *爆発律* の原理について述べましょう．Leanではこれは ``False.elim`` で表現され，任意の命題 ``P`` に対して ``False → P`` を成立させます．これは奇妙な原理のように思えるかもしれませんが，かなり頻繁に出てきます．何かしらの定理を証明する際にはよく場合分けを行いますが，その際ときおりそのうちの一つのケースが矛盾していることを示す場合があります．そのような場合，矛盾がゴールを成立させていることを主張する必要があり，これを行うことでようやく次のケースに移ることができます．（場合分けによる推論の例は :numref:`disjunction` で見ていきます．）
