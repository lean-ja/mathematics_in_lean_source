...
OMIT. -/
/- TEXT:
最後に, ``convert`` タクティクは結論がゴールに完全に一致しないような定理でもゴールに適用することができます．例えば ``1 < a`` から ``a < a * a`` を証明したいとしましょう．ライブラリには ``mul_lt_mul_right`` という定理があり，これを使えば ``1 * a < a * a`` を証明することができます．これを用いる方法の一つは，逆算してその形になるようにゴールを書き直すことでしょう．この代わりに ``convert`` タクティクは定理をそのまま適用でき，ゴールと定理の差分を埋めるのに必要な等式を，新たな証明すべきゴールとします．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
この例ではまた別の便利な技法を示しています．アンダースコアを含む式を ``apply`` すると，Lean はまずそれを自動的に埋めようとし，埋められない場合には単にそれが新しいゴールになります．
...
OMIT. -/
/- TEXT:
より興味深い定理として，もし ``s`` が ``a`` に， ``t`` が ``b`` に収束するならば， ``fun n ↦ s n + t n`` は ``a + b`` に収束することを示しましょう．形式的な証明を始める前に，紙とペンでどう証明するか明確にイメージしておくと良いでしょう．証明の方針は正の数 ``ε`` が与えられたとき，仮定を使ってそれ以降 ``s`` が常に ``a`` から ``ε / 2`` 以内に収まるような ``Ns`` と， それ以降 ``t`` が常に ``b`` から ``ε / 2`` 以内に収まるような ``Nt`` をそれぞれ求めることです．そして ``n`` が ``Ns`` と ``Nt`` の最大値以上であるとき， ``fun n ↦ s n + t n`` は ``a + b`` から ``ε`` 以内にあるはずです．以下ではこの方針を実装します．以下の証明を完成させてみましょう．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
:numref:`filters` では，Mathlibには収束をより一般的に扱うためのメカニズムが用意されていることを見ていきます．filter は始域と終域の特定の特徴を抽象化するだけではなく，様々なタイプの収束を抽象化します．
TEXT. -/
