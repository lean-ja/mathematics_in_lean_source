...
OMIT. -/
/- TEXT:
これで本格的な数学ができる準備が整いました．Leanでは実数の数列 :math:`s_0, s_1, s_2, \ldots` は関数 ``s : ℕ → ℝ`` で表すことができます．すべての :math:`\varepsilon > 0` に対して，数列上のずっと後ろの点が :math:`a` から :math:`\varepsilon > 0` 以内の範囲に収まる場合，このような数列は :math:`a` に *収束* すると言います．つまり，すべての :math:`n \ge N` に対して :math:`| s_n - a | < \varepsilon` となるような数 :math:`N` が存在するということです．Leanではこれは次の表に表現できます．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
``∀ ε > 0, ...`` という記法は ``∀ ε, ε > 0 → ...`` の便利な省略形で，また同じように ``∀ n ≥ N, ...`` も ``∀ n, n ≥ N →  ...`` の省略形です．そして ``ε > 0`` は ``0 < ε`` と， ``n ≥ N`` は ``N ≤ n`` として定義されることも覚えておきましょう．
...
OMIT. -/
/- TEXT:
本節では，収束のいくつかの性質を証明します．しかしその前に，等式を扱う際に役立つ3つのタクティクについて説明しましょう．まず初めに， ``ext`` タクティクは2つの関数が等しいことを証明する方法を与えてくれます．例えば :math:`f(x) = x + 1` と :math:`g(x) = 1 + x` は実数から実数への関数とします．このとき当たり前ですが，2つの関数はすべての :math:`x` に対して同じ値を返すため :math:`f = g` となります． ``ext`` タクティクは関数間の同値をこのように引数のすべての値に対して等しい値を返すことで証明することを可能にしてくれます．
TEXT. -/
-- QUOTE:
...
後で見ていきますが， ``ext`` は実はもっと汎用的で登場する変数の名前を指定することもできます．例えば上記の例で ``ext`` を ``ext u v`` に置き換えてみることができます．2つ目のタクティクは ``congr`` タクティクで，これは2つの式の同値性を式中の異なる部分を調整することで証明するものです．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ここでは ``congr`` タクティクが両辺の ``abs`` を剥がし， ``a = a - b + b`` の証明を残しています．
...
OMIT. -/
/- TEXT:
最後に ``convert`` タクティクは使用したい定理の結論がゴールに完全に一致しない場合でもゴールに定理を適用するために使われます．例えば ``1 < a`` から ``a < a * a`` を証明したいとしましょう．ライブラリには ``mul_lt_mul_right`` という定理があり，これを使えば ``1 * a < a * a`` を証明することができます．これを用いる場合，一つの方法としては逆算してその形になるようにゴールを書き直すことでしょう．この代わりに ``convert`` タクティクは定理をそのまま適用でき，その際にゴールに合わせる際に必要な等式を証明すべきゴールとして残します．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
この例ではまた別の便利な技法を示しています．アンダースコアを含む式を適用したとき，Leanがそれを自動的に埋められない場合には，Leanは単にそれを新しいゴールとして残してくれるのです．
...
OMIT. -/
/- TEXT:
以下では定数の数列 :math:`a, a, a, \ldots` が収束することを示しています．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
より興味深い定理として，もし ``s`` が ``a`` に， ``t`` が ``b`` に収束するならば， ``fun n ↦ s n + t n`` は ``a + b`` に収束することを示しましょう．この証明を始める前に，紙とペンによる明確な証明を頭に入れておくと良いでしょう．証明の方針は ``ε`` が ``0`` よりも大きいとすると，極限では ``s`` が ``a`` の ``ε / 2`` 以内に収まるような ``Ns`` と， ``t`` が ``b`` の ``ε / 2`` 以内に収まるような ``Nt`` を仮説を使って求めることです．そして ``n`` が ``Ns`` と ``Nt`` の最大値以上であるとき， ``fun n ↦ s n + t n`` は ``a + b`` の ``ε`` 以内にあるはずです．以下ではこの方針を実装しようとしているものです．完成できるか試してみましょう．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ヒントとして， ``le_of_max_le_left`` と ``le_of_max_le_right`` を用い， ``norm_num`` を使って ``ε / 2 + ε / 2 = ε`` を証明できます．また， ``|s n + t n - (a + b)|`` が ``|(s n - a) + (t n - b)|`` と等しいことを示すにあたっては ``congr`` が有用でしょう．これによって三角不等式を使うことができる形になります．ここで変数 ``s`` ， ``t`` ， ``a`` ， ``b`` は仮説から推測できるため，すべて暗黙の変数としました．
...
OMIT. -/
/- TEXT:
足し算の代わりに掛け算を使った同じような定理の証明は難しくなります．この証明のためにはまずいくつかの補助的な命題を証明することが必要です．もし ``s`` が ``a`` に収束するならば， ``fun n ↦ c * s n`` は ``c * a`` に収束することを示す次の証明も完成できるか見てみましょう．この証明にあたって ``c`` がゼロに等しいかどうかでケースを分けると便利です．ゼロのケースについては埋めておくので， ``c`` がゼロではない場合のケースを証明してみましょう．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
次の定理もこれはこれで面白いものです: 収束する数列が最終的に絶対値で有界になることを示しています．初めの部分は示しているので，残りを証明してみましょう．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
実際，この定理をさらに強くして，すべての ``n`` の値に対して成り立つ境界 ``b`` があることを主張することもできます．ただ，このバージョンは今の目的には十分強力であるため，より一般的に成り立つことは本説の最後で確認することとします．
...
OMIT. -/
/- TEXT:
次の補題は補助的なもので，もし ``s`` が ``a`` に収束し， ``t`` が ``0`` に収束するならば， ``fun n ↦ s n * t n`` は ``0`` に収束することを証明します．このためには前の定理を用いてある点 ``N₀`` を超えたところで ``s`` を境界付ける ``B`` を見つけます．このおおまかな方針が理解できたら以下の証明をやってみましょう．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ここまでできたのならおめでとうございます！私達は今，目的の定理の射程圏内にいます．次の証明で完成です．
TEXT. -/
-- QUOTE:
-- BOTH:
...
OMIT. -/
/- TEXT:
さらに難しい練習問題として，極限が一意であることを証明するスケッチを次のように埋めてみましょう．（もしいけそうなら証明のスケッチを削除してイチから証明してみてもいいでしょう．）
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
本節を終えるにあたってここまでの証明は一般化できることを述べておきましょう．例えば自然数について本説のここまでで用いてきた唯一の性質は，自然数の構造が ``min`` と ``max`` について半順序を持つことでした．ここで証明中の ``ℕ`` をすべて任意の線形順序 ``α`` に置き換えてもうまく行くことを確認できます:
TEXT. -/
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
:numref:`filters` では，Mathlibが収束をより一般的に扱うためのメカニズムを持っていることを見ます．それは始域と終域の特定の特徴を抽象化するだけではなく，収束の様々なタイプを抽象化することがわかります．
TEXT. -/
