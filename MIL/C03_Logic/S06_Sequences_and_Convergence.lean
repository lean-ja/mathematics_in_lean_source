...
OMIT. -/
/- TEXT:
これで本格的な数学ができる準備が整いました．Leanでは実数の数列 :math:`s_0, s_1, s_2, \ldots` は関数 ``s : ℕ → ℝ`` で表すことができます．すべての :math:`\varepsilon > 0` に対して，数列上のずっと後ろの点が :math:`a` から :math:`\varepsilon > 0` 以内の範囲に収まる場合，このような数列は :math:`a` に *収束* すると言います．つまり，すべての :math:`n \ge N` に対して :math:`| s_n - a | < \varepsilon` となるような数 :math:`N` が存在するということです．Leanではこれは次のように表現できます．
BOTH: -/
-- QUOTE:
...
後で見ていきますが， ``ext`` は実はもっと汎用的で，登場する変数の名前を指定することもできます．例えば上記の例で ``ext`` を ``ext u v`` に置き換えることができます．2つ目のタクティクは ``congr`` タクティクで，これは2つの式の同値性を式中の異なる部分を調整することで証明するものです．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
より興味深い定理として，もし ``s`` が ``a`` に， ``t`` が ``b`` に収束するならば， ``fun n ↦ s n + t n`` は ``a + b`` に収束することを示しましょう．この証明を始める前に，紙とペンによる明確な証明を頭に入れておくと良いでしょう．証明の方針は ``ε`` が ``0`` よりも大きいとすると，極限では ``s`` が ``a`` の ``ε / 2`` 以内に収まるような ``Ns`` と， ``t`` が ``b`` の ``ε / 2`` 以内に収まるような ``Nt`` を仮定を使って求めることです．そして ``n`` が ``Ns`` と ``Nt`` の最大値以上であるとき， ``fun n ↦ s n + t n`` は ``a + b`` の ``ε`` 以内にあるはずです．以下ではこの方針を実装しようとしているものです．この証明を完成させてみましょう．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ヒントとして， ``le_of_max_le_left`` と ``le_of_max_le_right`` を用い， ``norm_num`` を使うことで ``ε / 2 + ε / 2 = ε`` を証明できます．また， ``|s n + t n - (a + b)|`` が ``|(s n - a) + (t n - b)|`` と等しいことを示すにあたっては ``congr`` が有用でしょう．これによって三角不等式を使うことができる形になります．ここで変数 ``s`` ， ``t`` ， ``a`` ， ``b`` は仮定から推測できるため，すべて暗黙の変数としました．
...
OMIT. -/
/- TEXT:
足し算の代わりに掛け算を使った同じような定理の証明は難しくなります．この証明のためにはまずいくつかの補助的な命題を証明することが必要です．もし ``s`` が ``a`` に収束するならば， ``fun n ↦ c * s n`` は ``c * a`` に収束することを示す次の証明も完成させてみましょう．この証明にあたって ``c`` が0に等しいかどうかでケースを分けると便利です．0のケースについては埋めておくので， ``c`` が0ではない場合のケースを証明してみましょう．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
実際，この定理をさらに強くして，すべての ``n`` の値に対して成り立つ境界 ``b`` があることを主張することもできます．この事実はより一般的に成り立ちますが，このバージョンでも今の目的には十分強力であるため，一般化は本節の最後で確認することとします．
...
OMIT. -/
/- TEXT:
さらに難しい演習問題として，極限が一意であることを証明する以下のスケッチを完成させてみましょう．（もしいけそうなら証明のスケッチを削除してイチから証明してみてもいいでしょう．）
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
本節を終えるにあたってここまでの証明は一般化できることを述べておきましょう．例えば自然数について本節のここまでで用いてきた性質は，自然数の構造が ``min`` と ``max`` について半順序を持つことだけでした．ここで証明中の ``ℕ`` をすべて任意の線形順序 ``α`` に置き換えてもうまく行くことを確認できます:
TEXT. -/
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
:numref:`filters` では，Mathlibが収束をより一般的に扱うためのメカニズムを持っていることを見ます．その概念は始域と終域の特定の特徴を抽象化するだけではなく，収束の様々なタイプを抽象化します．
TEXT. -/
