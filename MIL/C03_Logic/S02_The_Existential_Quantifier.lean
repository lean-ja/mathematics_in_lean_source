...
OMIT. -/
/- TEXT:
この実際の値とその性質の証明の2つをまとめる方法はいくつかあります．まず1つ目として，存在量化子で始まるゴールがある場合， ``use`` タクティクを用いてその対象を提示することで，その性質を証明するというゴールが残ります．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
実際には， ``use`` タクティクは自動的に利用可能な仮定も使おうとします．
TEXT. -/
-- QUOTE:
example : ∃ x : ℝ, 2 < x ∧ x < 3 := by
...
あるいは，Leanの *匿名コンストラクタ* の記法を用いて，存在量化子の証明を構成することもできます．
TEXT. -/
-- QUOTE:
example : ∃ x : ℝ, 2 < x ∧ x < 3 :=
  have h : 2 < (5 : ℝ) / 2 ∧ (5 : ℝ) / 2 < 3 := by norm_num
...
OMIT. -/
/- TEXT:
ここで ``by`` を用いていないことに注目してください; ここでは明示的に証明項を与えているのです．それぞれ ``\<`` と ``\>`` で入力されている左と右の角括弧はLeanに与えられたデータを現在のゴールに適した構成でまとめるように指示してくれます．初めからタクティクモードに入ることなくこの表記を使うこともできます:
TEXT. -/
-- QUOTE:
example : ∃ x : ℝ, 2 < x ∧ x < 3 :=
  ⟨5 / 2, by norm_num⟩
...
OMIT. -/
/- TEXT:
これで存在についての命題の *証明をする* 方法がわかりました．しかしこのような命題はどのように *使う* のでしょうか？ある性質を持つ対象が存在することがわかっていれば，任意の対象に名前を付けて推論することができるはずです．例えば，前節で ``a`` が ``f`` の上界または下界であることをそれぞれ表していた ``FnUb f a`` と ``FnLb f a`` という述語を思い出してください．存在量化子を使えば具体的な上界・下界を指定せずに「 ``f`` は上界・下界を持つ」と言うことができます:
TEXT. -/
-- BOTH:
-- QUOTE:
...
``rcases`` タクティクは存在量化子の情報を展開します．匿名コンストラクタと同じ角括弧で書かれた ``⟨a, ubfa⟩`` のような注釈は *パターン* と呼ばれ，メインの引数を展開した結果として期待される情報を記述します．このときゴールは変更されません; 変更 *された* のは新しい対象と新しい仮説を使用してゴールを証明できるようになったことです．これは数学において一般的な推論の手法です: なにかしらの仮説から存在を主張、もしくは暗示されたときにその対象を展開し，それを使って他の何かの存在を証明するのです．
...
OMIT. -/
/- TEXT:
この手法を用いて次の例を証明してみましょう． ``fn_ub_add`` の証明でやったように前節のいくつかの例を名前付きの定理にしたり，引数を証明に直接入れたりすると証明に役立つかもしれません．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
実際，Leanは式と証明項中でのfunによるパターンマッチもサポートしています:
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
仮説から情報を展開する作業はとても重要であるため，LeanとMathlibはこのための方法をいくつも提供しています．例えば ``obtain`` タクティクはこの意味として直感的な記法を提供します:
TEXT. -/
-- QUOTE:
example (ubf : FnHasUb f) (ubg : FnHasUb g) : FnHasUb fun x ↦ f x + g x := by
  obtain ⟨a, ubfa⟩ := ubf
...
OMIT. -/
/- TEXT:
1つ目の ``obtain`` 命令は与えられたパターンをもとに ``ubf`` の「中身」をマッチングさせ，その構成要素を名前のついた変数に代入すると考えてください． ``rcases`` と ``obtain`` はどちらも引数を ``destruct`` すると言われますが， ``rcases`` は使用すると引数に渡した ``ubf`` を文脈から消してしまうのに対し， ``obtain`` ではそのまま残るという小さな違いがあります．
...
OMIT. -/
/- TEXT:
1つ目の例にて ``cases ubf`` の後ろにカーソルを置くと，このタクティクによってLeanが ``intro`` というタグを付けた1つのゴールが生成されていることがわかるでしょう．（この特別な名前は存在についての証明を組み立てる公理的なLeanのプリミティブな内部的な名前に由来しています．）これに続く ``case`` タクティクがこのコンポーネントに名前を付けています．2つ目の例も似ていますが， ``case`` の代わりに ``next`` を使うことで， ``intro`` に言及することを避けることができます．最後の2つの例の ``match`` という単語はここで行っていることが計算科学者が「パターンマッチング」と呼んでいることを強調しています．3つ目の証明は ``by`` で始まっていることに注目してください．タクティクバージョンの ``match`` では矢印の右側ではタクティクによる証明が来ることを期待しています．最後の例は証明項です: ここではタクティクは一つも見当たりません．
...
OMIT. -/
/- TEXT:
本書ではこれ以降 ``rcases`` と ``rintros`` ， ``obtain`` を存在量化子を扱う方法として好ましいものとして説明します．しかし別の構文を見ておくことも損はないでしょう．特にもし仮に計算科学者と一緒にいるのでしたらなおさらです．
...
OMIT. -/
/- TEXT:
``rcases`` の使い道の一つを例示するにあたって，数学に置いてすでに語り尽くされてきた事実を証明しましょう: もし2つの整数 ``x`` と ``y`` がそれぞれ2乗の和として表せられる場合，その積 ``x * y`` も同様に表すことができます．実際にはこの命題は整数に限らず，任意の可換環に対しても成り立ちます．次の例にて， ``rcases`` は2つの存在量化子を一度に展開しています．そして ``x * y`` を2乗の和として表現するために必要な特定の値をリストとして ``use`` 文に与え， ``ring`` タクティクを使ってそれらの値が条件を満たすことを確認します．
TEXT. -/
section
