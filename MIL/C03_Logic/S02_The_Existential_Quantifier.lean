...
OMIT. -/
/- TEXT:
この証明ではたいした洞察を得られませんでしたが，この事実に動機づけられる例が一つあります． **ガウス整数** （Gaussian integer）とは :math:`a + bi` の形式で表される数で，ここで :math:`a` と :math:`b` は整数で :math:`i = \sqrt{-1}` です．ガウス整数 :math:`a + bi` の **ノルム** （norm）は定義から :math:`a^2 + b^2` に一致します．したがってガウス整数のノルムは2乗の和で，逆にどんな2乗の和もノルムとして表すことができます．上記の定理はガウス整数の積のノルムはそれらのノルムの積であるという事実を反映しています: もし :math:`x` が :math:`a + bi` のノルムで :math:`y` が :math:`c + di` のノルムであるならば， :math:`xy` は :math:`(a + bi) (c + di)` のノルムとなります．上記の不可解な証明は形式化するのが最も簡単な証明が最も明瞭なものになるとは限らないという事実を示しています． :numref:`section_building_the_gaussian_integers` ではガウス整数を定義し，それを用いて別の証明を行う方法を提供します．
...
もう一つの重要な例として，関数 :math:`f : \alpha \to \beta` が **全射** （surjective）であるとは終域 :math:`\beta` のすべての :math:`y` について， :math:`f(x) = y` を満たす始域 :math:`\alpha` にある :math:`x` が存在することです．この命題では全称量化子と存在量化子の両方を含むことに注目してください．これが次の例で ``intro`` と ``use`` の両方を用いている理由です．
TEXT. -/
-- BOTH:
section
