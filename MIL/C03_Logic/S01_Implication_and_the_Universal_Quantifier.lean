...
OMIT. -/
/- TEXT:
ここでさらに例を挙げましょう．:math:`\Bbb R` から :math:`\Bbb R` への関数 :math:`f` が *偶関数* であるとはすべての :math:`x` に対して :math:`f(-x) = f(x)` となることを指し， *奇関数* であるとはすべての :math:`x` に対して :math:`f(-x) = -f(x)` となることを指します．以下の例ではこの2つの概念を形式的に定義し，またこれらについて事実を一つ証明します．残りの証明は読者自身で完成させてみましょう．
TEXT. -/
-- QUOTE:
-- BOTH:
