...
OMIT. -/
/- TEXT:
Mathlibは集合を操作するにための優れたライブラリを有しています．ところでLeanは集合論を基礎に置いていないことを思い出してください，そのためここで言うところの集合は何かしらの型 ``α`` の数学的対象のあつまりというありふれた意味合いになります．もし ``x`` が型 ``α`` で， ``s`` が ``Set α`` 型であるとき， ``x ∈ s`` は ``x`` が ``s`` の要素であることを主張する命題となります．別の型 ``β`` の ``y`` については，式 ``y ∈ s`` は意味をなしません．ここでいう「意味をなさない」とは「型を持たないため，Leanとして合法な（well-formed）命題として受け入れられない」という意味です．これは例えばツェルメロ-フレンケルの集合論（ZF）では ``a ∈ b`` がすべての数学的対象 ``a`` と ``b`` に対して合法であることと対照的です．例えば ``sin ∈ cos`` もZFでは合法です．この集合論の欠点は無意味な式を検知して証明を支援することを意図した証明支援系として上記の集合論的な基礎を使用しない重要な動機となっています．Leanにおいて ``sin`` は ``ℝ → ℝ`` の型を持ち，``cos`` も ``ℝ → ℝ`` の型であり，これはたとえ定義を展開したとしても ``Set (ℝ → ℝ)`` とは等しくないため，``sin ∈ cos`` という命題は意味をなしません．一方でLeanを使って集合論そのものに取り組むこともできます．例えばZFの公理から連続体仮定が独立していることはLeanで形式化されています．しかしこのような集合論のメタ理論は本書の範囲を完全に超えています．
