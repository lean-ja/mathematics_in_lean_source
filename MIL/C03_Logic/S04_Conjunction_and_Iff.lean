...
この例では， ``assumption`` タクティクがLeanにゴールを解決する仮定を見つけるように指示しています．最後の ``rw`` で ``≤`` の反射性を適用してゴールを終了させていることに注意してください．以下は先程の例を無名コンストラクタを用いた別解です．1つ目の例は証明項中で ``by`` キーワードを用いてタクティクモードにしている巧妙な例です．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
連言を証明するのではなく *使う* にはその連言の証明を2つに展開することで行えます．これを行うには存在量化子でやっていたように ``rcases`` タクティクや ``rintro`` ， ``fun`` のパターンマッチを使うことができます．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
存在量化子で使っていた ``obtain`` と同様にパターンマッチを行う ``have`` もあります:
TEXT. -/
-- QUOTE:
example {x y : ℝ} (h : x ≤ y ∧ x ≠ y) : ¬y ≤ x := by
  have ⟨h₀, h₁⟩ := h
...
OMIT. -/
/- TEXT:
``rcases`` とは対称的に，ここで用いた ``have`` タクティクは ``h`` を文脈に残します．また，今後本書では使いませんが，計算機科学者が言うところのパターンマッチの構文の例を今一度挙げましょう:
TEXT. -/
-- QUOTE:
example {x y : ℝ} (h : x ≤ y ∧ x ≠ y) : ¬y ≤ x := by
  cases h
...
OMIT. -/
/- TEXT:
存在量化子の使い方とは対称的に，仮定 ``h : A ∧ B`` の2つの構成要素を取り出すために ``h.left`` と ``h.right`` ，あるいは ``h.1`` と ``h.2`` と書くことができます．
TEXT. -/
-- QUOTE:
example {x y : ℝ} (h : x ≤ y ∧ x ≠ y) : ¬y ≤ x := by
  intro h'
...
OMIT. -/
/- TEXT:
``rintro`` と ``rcases`` の無名コンストラクタを使うことで ``∃`` と ``∧`` をネストすることができます:
TEXT. -/
-- QUOTE:
example : ∃ x : ℝ, 2 < x ∧ x < 4 :=
...
OMIT. -/
/- TEXT:
Leanでは，双方向の含意は二重生活を送っています．まず，連言のように2つの部分を別々に使うことができます．しかしその一方で，Leanはこれを命題環の反射的，対称的，そして推移的な関係であることも知っており， ``calc`` や ``rw`` と一緒に使うこともできます．こうして命題を同値なものに書き換えるのは便利なことが多いです．次の例では， ``abs_lt`` を使って ``|x| < y`` の形の式を同値な ``- y < x ∧ x < y`` に書き換え，その次の例では ``Nat.dvd_gcd_iff`` を用いて ``m ∣ Nat.gcd n k`` の形式の式を同値な ``m ∣ n ∧ m ∣ k`` に置き換えています．
TEXT. -/
section
...
OMIT. -/
/- TEXT:
本節での最後の演習問題は連言と双方向の含意を練習するためのものです． *半順序* は推移的で反射的で反対称的な二項関係であることを思い出してください．これに対してより弱い概念である *前順序* は単に反射的で推移的な関係のことです．任意の前順序 ``≤`` について，Leanは関連する狭義の前順序``a < b`` を ``a < b ↔ a ≤ b ∧ ¬ b ≤ a`` で公理化しています．もし ``≤`` が半順序ならば， ``a < b`` は ``a ≤ b ∧ a ≠ b`` と等価であることを示しましょう:
TEXT. -/
-- BOTH:
section
-- QUOTE:
...
論理演算以外では ``le_refl`` と ``le_trans`` 以外は必要ではありません．ここで ``≤`` が前順序であることだけを仮定したとしても狭義の順序が非反射的で推移的であることを示しましょう．2つ目の例では， ``<`` を ``≤`` と ``¬`` を用いて表現するために便宜上 ``rw`` ではなく単純化のコマンドを用います．この単純化については後ほど説明しますが，ここではこれによって指定された補題がたとえ異なる値についてインスタンス化する必要がある場合でも繰り返し使用されるという事実のみを用います．
TEXT. -/
-- BOTH:
section
-- QUOTE:
