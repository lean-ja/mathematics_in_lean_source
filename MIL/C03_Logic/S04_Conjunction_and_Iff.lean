...
OMIT. -/
/- TEXT:
「かつ」を表すのに使われている連言の記号 ``∧`` はもうすでに見てきました． ``constructor`` タクティクを使うことで ``A`` を証明し，次に ``B`` を証明することで ``A ∧ B`` という形の命題を証明できます．
TEXT. -/
-- QUOTE:
...
この例では， ``assumption`` タクティクがLeanにゴールを解決する仮定を見つけるように指示しています．最後の ``rw`` で ``≤`` の反射性を適用してゴールを終了させていることに注意してください．以下は先程の例を匿名コンストラクタを用いた別解です．1つ目の例は証明項中で ``by`` キーワードを用いてタクティクモードにしている巧妙な例です．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
連言を証明するのではなく *使う* にはその連言の証明を2つに展開することで行えます．これを行うには存在量化子でやっていたように ``rcases`` タクティクや ``rintro`` ， ``fun`` のパターンマッチングを使うことができます．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
存在量化子で使っていた ``obtain`` と同様にパターンマッチングを行う ``have`` もあります:
TEXT. -/
-- QUOTE:
example {x y : ℝ} (h : x ≤ y ∧ x ≠ y) : ¬y ≤ x := by
  have ⟨h₀, h₁⟩ := h
...
OMIT. -/
/- TEXT:
``rcases`` とは対称的に，ここで用いた ``have`` タクティクは ``h`` を文脈に残します．また，今後本書では使いませんが，計算科学者が言うところのパターンマッチングの構文の例を今一度挙げましょう:
TEXT. -/
-- QUOTE:
example {x y : ℝ} (h : x ≤ y ∧ x ≠ y) : ¬y ≤ x := by
  cases h
...
OMIT. -/
/- TEXT:
存在量化子の使い方とは対称的に，仮説 ``h : A ∧ B`` の2つの構成要素を取り出すために ``h.left`` と ``h.right`` ，あるいは ``h.1`` と ``h.2`` と書くことができます．
TEXT. -/
-- QUOTE:
example {x y : ℝ} (h : x ≤ y ∧ x ≠ y) : ¬y ≤ x := by
  intro h'
...
OMIT. -/
/- TEXT:
``rintro`` と ``rcases`` の匿名コンストラクタを使うことで ``∃`` と ``∧`` をネストすることができます:
TEXT. -/
-- QUOTE:
example : ∃ x : ℝ, 2 < x ∧ x < 4 :=
...
OMIT. -/
/- TEXT:
1つ目の例では， ``constructor`` の後ろのセミコロンによってLeanに2つのゴールのどちらにも ``norm_num`` タクティクを用いるように指示しています．
...
OMIT. -/
/- TEXT:
Leanでは ``A ↔ B`` は ``(A → B) ∧ (B → A)`` で定義されて *いません* が，このようにな定義であっても良いはずですし，実際にほぼ同じ振る舞いをします．ここまでですでに ``h : A ↔ B`` の両方向として ``h.mp`` と ``h.mpr`` ，もしくは ``h.1`` と ``h.2`` を書けることを見てきました．また ``cases`` とそれに類するものも使えます．「〜かつまたそのときに限る」という命題を証明するには，連言を証明するときと同じように， ``constructor`` や角括弧を使うことができます．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
最後の証明項は複雑でよくわからないことになっています．このような式を書くときにアンダースコアを使うと，Leanが何を期待していることがわかることを覚えておきましょう．
...
OMIT. -/
/- TEXT:
さらに興味深い練習問題として，任意の2つの実数 ``x`` と ``y`` に対して， ``x^2 + y^2 = 0`` が ``x = 0`` と ``y = 0`` の場合にのみ成り立つことを示しましょう．ここでは ``linarith`` ， ``pow_two_nonneg`` ， ``pow_eq_zero`` を使って補助的な補題を証明することをお勧めします．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
Leanでは，双方向の含意は二重生活を送っています．連言のように2つの部分を別々に使うことができます．しかし，Leanはこれを命題環の反射的，対称的，そして推移的な関係であることも知っており， ``calc`` や ``rw`` と一緒に使うこともできます．こうして命題を同値なものに書き換えるのは便利なことが多いです．次の例では， ``abs_lt`` を使って ``|x| < y`` の形の式を同値な ``- y < x ∧ x < y`` に書き換え，その次の例では ``Nat.dvd_gcd_iff`` を用いて ``m ∣ Nat.gcd n k`` の形式の式を同値な ``m ∣ n ∧ m ∣ k`` に置き換えています．
TEXT. -/
section
...
OMIT. -/
/- TEXT:
以下の定理で ``rw`` を使って，和の逆数が非減少関数ではないことを簡単に証明できるか試してみましょう．（ ``push_neg`` が定義を展開してくれないため，定理の証明にある ``rw [Monotone]`` が必要であることに注意してください．）
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
本節での最後の練習問題は連言と双方向の含意を練習するためのものです． *半順序* は推移的で反射的で反対称的な二項関係であることを思い出してください．これに対してより弱い概念として *前順序* は単に反射的で推移的な関係のことです．任意の前順序 ``≤`` について，Leanは関連する厳密な前順序``a < b`` を ``a < b ↔ a ≤ b ∧ ¬ b ≤ a`` で公理化しています．もし ``≤`` が半順序ならば， ``a < b`` は ``a ≤ b ∧ a ≠ b`` と等価であることを示しましょう:
TEXT. -/
-- BOTH:
section
-- QUOTE:
...
論理演算意外では ``le_refl`` と ``le_trans`` 以外は必要ではありません．ここで ``≤`` が前順序であることだけを仮定したとしても厳密な順序が反反射的で推移的であることを示しましょう．2つ目の例では， ``<`` を ``≤`` と ``¬`` を用いて表現するために便宜上 ``rw`` ではなく簡約化のコマンドを用います．この簡約化については後ほど説明しますが，ここではこれによって指定された補題がたとえ異なる値についてインスタンス化する必要がある場合でも繰り返し使用されるという事実のみを用います．
TEXT. -/
-- BOTH:
section
-- QUOTE:
