...
OMIT. -/
/- TEXT:
本節での最後の演習問題は連言と双方向の含意を練習するためのものです． *半順序* は推移的で反射的で反対称的な二項関係であることを思い出してください．これに対してより弱い概念である *前順序* は単に反射的で推移的な関係のことです．任意の前順序 ``≤`` について，Leanは対応する狭義の前順序 ``a < b`` を ``a < b ↔ a ≤ b ∧ ¬ b ≤ a`` で公理化しています．もし ``≤`` が半順序ならば， ``a < b`` は ``a ≤ b ∧ a ≠ b`` と等価であることを示しましょう:
TEXT. -/
-- BOTH:
section
-- QUOTE:
