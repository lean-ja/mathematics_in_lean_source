...
OMIT. -/
/- TEXT:
このように段階的なフィードバックを受けながら細かいステップを積み重ねて証明を組み立てることのできる能力は非常に強力です．このためタクティクによる証明はしばしば証明項を書き下すことより簡単で早くなります．とはいえ二つのモードの間に明確な線引きは存在しません:上記の例で ``by rw [hk, mul_left_comm]`` というフレーズで実演したように，タクティクを用いる証明は項による証明中に入れることが可能です．そして反対に，この後見ていくようにタクティクによる証明中で短い項による証明を用いることも時として有用になります．とはいえ，本書ではタクティクの使用に重点を置くこととしています．
...
OMIT. -/
/- TEXT:
上記の我々の例について，タクティクによる証明はさらに一行にまとめることも可能です．
TEXT. -/
-- QUOTE:
example : ∀ m n : Nat, Even n → Even (m * n) := by
...
OMIT. -/
/- TEXT:
そしてそうなるために読者がするべきことは，とにかくやってみて，演習問題に取り組み，Zulipで質問をし，そして楽しむことです．しかし先に注意しておくこととして，対話的な定理証明は読者が数学や数学的な証明に際して根本的に新しい方法で考える意欲を書き立てることでしょう．読者の人生はもはや以前のものとは全く異なることでしょう．
