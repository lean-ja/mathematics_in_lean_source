...
OMIT. -/
/- TEXT:
本書のゴールは，Lean4の対話的な証明アシスタントを用いて数学を定式化する方法を読者に伝えることです．これにあたって読者にはある程度数学知識があることを前提にしますが，それほど多くは必要ではありません．本書では数論から測度論，解析学までの内容を取り上げますが，ここではこれらの分野の初歩的な側面のみに焦点をあてます．もし上記の中に馴染みのない分野があったとしても，順を追って本書を読み進めていくことで理解できることを期待しています．また，本書では形式手法のバックグラウンドがあることも前提としません．形式化は一種のコンピュータープログラミングと見なすことが出来ます:本書において数学の定義，定理，証明はLeanが理解できるようにプログラミング言語のような規則化された言語で記述します．こうすることによりLeanは与えられた式を解釈してその式が適格であることを保証し，そして最終的に「証明の正しさ」の証明をフィードバックとして提供します．
...
OMIT. -/
/- TEXT:
本書ではさらに挑戦し甲斐のあるエクササイズをたくさん掲載しています．しかし焦ってはいけません！Leanは数学を対話的に *実行する* ものであり，読むためのものではありません．実際に練習をこなすことがLean学習における必要な経験の中心です．全ての練習問題をこなす必要はありません．もし関連するスキルが身に着いたと感じたら，自由に次に進んでください．そして読者はいつでも自身の回答と各セクションに関連する ``solutions`` フォルダにある回答と比較することが出来ます．
TEXT. -/
