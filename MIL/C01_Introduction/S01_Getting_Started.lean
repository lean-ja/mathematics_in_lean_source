...
本書のゴールは，Lean4の対話的な証明アシスタントを用いて数学を定式化する方法を読者に伝えることです．
これにあたって読者にはある程度数学知識があることを前提にしますが，それほど多くは必要ではありません．
本書では数論から測度論，解析学までの内容を取り上げますが，ここではこれらの分野の初歩的な側面のみに焦点をあてます．
もし上記の中に馴染みのない分野があったとしても，順を追って本書を読み進めていくことで理解できることを期待しています．
また，本書では形式手法のバックグラウンドがあることも前提としません．
形式化は一種のコンピュータープログラミングと見なすことが出来ます:
本書において数学の定義，定理，証明はLeanが理解できるようにプログラミング言語のような規則化された言語で記述します．
こうすることによりLeanは与えられた式を解釈してその式が適格であることを保証し，そして最終的に「証明の正しさ」の証明をフィードバックとして提供します．
TEXT. -/
/- OMIT:
...
Leanについての詳細な情報は`Leanのプロジェクトページ<https://leanprover.github.io>`_や`Leanコミュニティ<https://leanprover-community.github.io/>`_で得られます．
このチュートリアルはLeanの大規模で，今もなお成長し続けているライブラリである*Mathlib*に基づいています．
またもしまだ参加していないのでしたら，`Lean Zulip online chat group <https://leanprover.zulipchat.com/>`_に参加することを強く推奨します．
そこはLeanの熱狂的なファンによる誰でも歓迎する活気に満ちたコミュニティで，喜んで読者の質問に答えたり精神的なサポートをしてくれるでしょう．
TEXT. -/
/- OMIT:
...
1. Lean4とVSCodeを`インストール方法<https://leanprover-community.github.io/get_started.html>`_に従ってインストールします．

2. `git <https://git-scm.com/>`_がインストールされていることを確認してください．

3. `こちらの手順<https://leanprover-community.github.io/install/project.html#working-on-an-existing-project>`_に従って``mathematics_in_lean``リポジトリをフェッチしてVSCodeで開いてください．

4. 本書の各セクションには例題や練習問題を含むLeanファイルが対応付けられています．
   これらのファイルは``MIL``フォルダ内にチャプタごとで格納されています．
   本書ではこのフォルダをコピーし，そこで動作確認や練習を行うことを強くお勧めします．
   これによりオリジナルのファイルはそのまま残り，リポジトリの変更に伴う更新も容易になります．（詳細は下記を参照）
   読者はこのコピーに``my_files``等の名前をつけ、これを使って新たに独自のLeanファイルを追加することもできます．
TEXT. -/
/- OMIT:
...
2. 上記手順で現れたバーで``Lean 4: Open Documentation View``と打ち込み，エンターキーを押下します．
   （もしくはメニュー内で上記の文字列が選択されてハイライトされている場合はそのままエンターキーを押下することも可能です．）
...
また，LeanとVSCodeを`Gitpod <https://gitpod.io/>`_を使ってクラウド上で実行することも可能です．
Mathematics in Leanについてこの方法を行うにはGithub上の`プロジェクトページ<https://github.com/leanprover-community/mathematics_in_lean>`_に手順があります．
この場合も前述したとおり`MIL`フォルダの複製上で学習を行うことをお勧めします．
TEXT. -/
/- OMIT:
...
本書とこれに関連するリポジトリは現在も開発中です．
リポジトリを更新するには``mathematics_in_lean``フォルダ内で``git pull``と打ち込み，``lake exe cache get``を実行します．
（この手順は``MIL``フォルダ内のコンテンツを変更していないことを仮定しています．これがフォルダをコピーすることを推奨している理由です．）
TEXT. -/
/- OMIT:
...
``MIL``フォルダ内の練習問題は教科書中にある解説や指示，ヒントを読みながら取り組むことを想定しています．
また本書ではしばしば次のような例を記載しています:
TEXT. -/
-- QUOTE:
#eval "Hello, World!"
-- QUOTE.
...
このような例を関連するLeanファイル中に見つけることができるでしょう．
この行をクリックすると，VSCodeは``Lean Goal``ウィンドウ内にLeanによるフィードバックを表示し，
また``#eval``コマンドにカーソルを合わせるとポップアップでこのコマンドに対するLeanの出力結果を表示します．
試しにこのファイルを編集して挙動を確認することをお勧めします．
TEXT. -/
/- OMIT:
...
本書ではさらに挑戦し甲斐のあるエクササイズをたくさん掲載しています．
しかし焦ってはいけません！
Leanは数学を対話的に*実行する*ものであり，読むためのものではありません．
実際に練習をこなすことがLean学習における必要な経験の中心です．
全ての練習問題をこなす必要はありません．もし関連するスキルが身に着いたと感じたら，自由に次に進んでください．
そして読者はいつでも自身の回答と各セクションに関連する``solutions``フォルダにある回答と比較することが出来ます．
TEXT. -/
