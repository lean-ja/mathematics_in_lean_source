...
OMIT. -/
/- TEXT:
測度論の発展における最初の要素は *可測* 集合と呼ばれる集合の :math:`\sigma`-代数の概念です．型クラス ``MeasurableSpace`` はこのような構造を持つ型を備えるためのものです．まず集合 ``empty`` と ``univ`` は可測です．可測集合の補集合も可測であり，可測集合の可算和または可算交差も可測です．これらの公理が冗長であることに注意してください; ``#print MeasurableSpace`` とタイプすると，Mathlibが使っている公理が表示されます．以下の例で示すように，可算性の仮定は ``Encodable`` 型クラスを使って表現することができます．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ある型が可測であれば，その型を測ることができます．書籍などにおいて， :math:`\sigma`-代数を備えた集合（または型）に対する測度とは，可測集合から拡張非負実数への関数であり，可算非交和上で加法的です．Mathlibでは，ある集合への測度の適用を記述するたびに可測の仮定を持ち出したくはありません．そこで， ``s`` を含む可測な集合の測度を最小値として，任意の集合 ``s`` に測度を拡張しています．もちろん，多くの補題では依然として可測の仮定を必要としますが，すべてではありません．
BOTH: -/
-- QUOTE:
open MeasureTheory
...
OMIT. -/
/- TEXT:
ある型に測度が紐づけられると，ある特性 ``P`` が *ほとんどいたるところ* で保持されるとは，その特性が失敗する要素の集合が測度0であることを指します．ほとんどいたるところで保持される特性の集合はフィルタを形成しますが，Mathlibはある特性がほとんどいたるところで保持されることを言うための特別な記法を導入しています．
EXAMPLES: -/
-- QUOTE:
