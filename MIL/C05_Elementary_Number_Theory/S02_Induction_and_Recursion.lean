...
OMIT. -/
/- TEXT:
このライブラリ中の定義は ``#check Nat`` と記述し、識別子 ``Nat`` を ``ctrl+クリック`` することで見つけることができます。このコマンドは ``Nat`` が2つのコンストラクタ ``zero : Nat`` と ``succ : Nat → Nat`` から自由かつ帰納的に生成されるデータ型であることを示しています。もちろん、ライブラリは ``nat`` と ``zero`` に対してそれぞれ ``ℕ`` と ``0`` という記法を導入しています。（数値は二進数表現に変換されますが、今はその詳細を気にする必要はありません。）
...
OMIT. -/
/- TEXT:
ここで「自由」という言葉は現役の数学者にとって ``Nat`` 型が要素 ``zero`` と ``zero`` をその像にふくまない単射な後続関数 ``succ`` を持つことを指します。
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
この構文には慣れが必要です。最初の行に ``:=`` が無いことに注目してください。そして次の2行は再帰的定義の基本ケースと帰納的なステップを展開しています。以下の等式は定義上保持されるため式変形ではLeanが勝手に展開等を行ってくれますが、 ``fac`` を ``simp`` や ``rw`` に与えることで手動で使用することもできます。
EXAMPLES: -/
-- QUOTE:
example : fac 0 = 1 :=
  rfl
...
OMIT. -/
/- TEXT:
この階乗関数はMathlibではすでに ``Nat.factorial`` として定義されています。ここでも再度 ``#check Nat.factorial`` と入力し、 ``ctrl+クリック`` することでこの関数の定義にジャンプすることができます。本書では説明のため、例題では ``fac`` を引き続き使用します。 ``Nat.factorial`` の定義の前にある ``@[simp]`` という注釈は、簡約化がデフォルトで使用する恒等式のデータベースにそこで定義した等式を追加することを指示します。
...
OMIT. -/
/- TEXT:
帰納法の原理は、自然数に関する一般的な命題を証明するには、その命題が0について成り立ち、また自然数 :math:`n` について成り立つときはいつでも :math:`n + 1` についても成り立つことを証明すれば良いというものです。そのため、以下の証明の ``induction' n with n ih`` という行は2つのゴールを生み出します。1つ目は ``0 < fac 0`` を証明することであり、2つ目は ``ih : 0 < fac n`` という仮定から ``0 < fac (n + 1)`` の証明が求められます。 ``with n ih`` というフレーズは帰納法で用いられる仮説の変数と仮定に名前をつけるためのもので、好きな名前をつけることができます。
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
``induction`` タクティクは賢いため、帰納変数に依存する帰納仮説を帰納仮説の一部に含めることができます。次の例でこれがなんのことか見てみましょう。
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
次の例では階乗関数の粗い下界を示します。証明のはじめに場合分けをすると簡単になるため、証明の残りは :math:`n = 1` の場合から始まるようにします。帰納法による証明で論証を完成してみましょう。
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
有限の和や積を含む恒等式を証明するために帰納法がよく用いられます。Mathlibは ``Finset.sum s f`` という式を定義しています。ここで ``s : Finset α`` は ``α`` 型の要素の有限集合であり、 ``f`` は ``α`` で定義される関数です。 ``f`` の余域は零要素を持つ可換で結合的な加法をサポートする任意の型です。 ``Algebra.BigOperators.Basic`` をインポートして ``open BigOperators`` コマンドを実行することで、より示唆的な表記である ``∑ x in s, f x`` を使うことができます。もちろん有限積についても同様の操作と表記法があります。
...
OMIT. -/
/- TEXT:
各ペアの1つ目の等式については定義的に成り立ちます。つまり証明を ``rfl`` で置き換えることができます。
...
OMIT. -/
/- TEXT:
簡約化のルールとして ``mul_comm`` がふくまれていることはコメントに値します。 ``x * y = y * x`` を簡約しようとすると普通は無限にループしてしまうことになるため、簡約化に追加するのは危険だと思われるでしょう。Leanの簡約化はその点を十分に認識しており、ある程度固定した上で任意の順序で項を並べたときに結果として得られる項の値が小さくなる場合にのみこの規則を適用するようにしています。次の例では ``mul_asso`` 、 ``mul_comm`` 、 ``mul_left_comm`` の3つのルールを使って簡約すると括弧の位置や変数の順序まで同じようにできることを示しています。
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
大まかには、括弧を右に押し出し、両辺の式が同じ標準的な順序になるまで並べ替えることでこのルールは機能します。これらのルールとそれに対応する加法のルールを使って簡約するのは便利なテクニックです。
...
OMIT. -/
/- TEXT:
和の恒等式の話に戻ると、 :math:`n` までの自然数の和が :math:`n (n + 1) / 2` となることの証明は次のようになります。証明の最初のステップでは、まず分母を消去します。これは恒等式を形式化する際に一般的に便利です。なぜなら除算は一般的に副次的な条件を持つからです。（同様に可能な限り自然数に対して減算を使わないようにすることも有効です。）
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
Leanのコアのライブラリでは、加法と乗法は再帰定義を使って定義されており、それらについての基本的な性質は帰納法を使って確立されています。もしこのような基礎的な話題に興味があるなら、乗法と加法の可換性と結合性、乗法と加法の分配性の証明に取り組むのも楽しいでしょう。これを自然数のコピーを用いて行うには以下のような流れになります。ここで ``MyNat`` で ``induction`` を使うことができることに注意してください。Leanは賢いので関連する帰納法の原理（もちろん ``Nat`` の原理と同じです）が使えることを知っています。
