...
OMIT. -/
/- TEXT:
これにあたっては普通の帰納法では不十分です．ここで必要なのはすべての自然数 :math:`n` が性質 :math:`P` を持つことを証明するために，すべての自然数 :math:`n` についてもし :math:`P` が :math:`n` より小さいすべての値で成り立つならば， :math:`n` でも成り立つことで示すという *強帰納法* です．Leanでは，この原理を ``Nat.strong_induction_on`` と呼び， ``with`` キーワードを使って帰納法のタクティクにこれを使うように指示することができます．この場合，ベースケースは存在せず，一般的な帰納法のステップにふくまれることに注意してください．
...
OMIT. -/
/- TEXT:
議論の流れは単純に以下のとおりです． :math:`n ≥ 2` を仮定し， :math:`n` が素数であればそれでおしまいです．もし素数でなければ，素数の特徴の1つである，素数の非自明な因数 :math:`m` を帰納法の仮説に適用することができます．次の証明を一行ずつ読めばこれがどのような役割を演じているかわかるでしょう． ``ih`` の表現が分かりづらい場合は ``dsimp at ih`` とすることで簡約することもできます．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
この補題を使って素数 ``p`` が素数の有限集合の積を分割する場合，そのうちの1つを分割することを示すことができます．Mathlibは有限集合の帰納法について便利な原理を提供しています．任意の有限集合 ``s`` の性質が成り立つことを示すには，空集合でその性質が成り立ち，新しい元 ``a ∉ s`` を1つ追加してもその性質が保たれることを示すことで出来るというものです．この原理は ``Finset.induction_on`` として知られています．これを使うように帰納法のタクティクに指示する際には， ``a`` と ``s`` の名前と帰納法のステップの仮定 ``a ∉ s`` の名前，帰納的仮説の名前を指定することもできます．式 ``Finset.insert a s`` は ``s`` と ``a`` の単集合の和を表します．恒等式 ``Finset.prod_empty`` と ``Finset.prod_insert`` は積に関連する書き換えルールです．以下の証明では最初の ``simp`` が ``Finset.prod_empty`` を適用しています．帰納法がどのように展開され，そして完成されるかを見るために証明の冒頭から順を追って確認してください．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
これで素数が無限にあることについての別の形式化を証明できます．すなわち，任意の ``s : Finset ℕ`` について，``s`` の元ではない素数 ``p`` が存在します．矛盾を導くために，すべての素数が ``s`` に存在することを仮定し，そこからすべての素数のみを含む集合 ``s'`` に切り捨てます．この集合の積を取り，1を加え，その結果の素因数を求めることで期待している矛盾が導かれます．下のスケッチを完成させましょう．最初の ``have`` の証明では ``Finset.prod_pos`` を使うことができます．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
無限に多くの素数が存在するということの2つ目の証明については4を法とした3に合同な素数が無限に存在することを示す別バージョンが存在します．この事実の証明の流れは次のようになります．まず，2つの数 :math:`m` と :math:`n` の積が4を法として3に合同である場合，2つの数のどちらかが4を法として3に合同であることに気づきます．そして結局の所どちらも奇数でなければならず，もし両方共4を法として1に合同であればその積も同じとなります．この観察を使って2より大きいある数が4を法として3に合同であるならば，その数は4を法として3に合同である素因数を持つことを示すことができます．
...
OMIT. -/
/- TEXT:
「 ``n`` が4を法として3に合同である」という命題は ``n % 4 = 3`` と表すことができます．以下の例と定理はこの後で使うこの関数についての事実を要約したものです．1つ目の名前付きの定理は先程利用した小さい数についてのケース分割による推論の別の例です．2つ目の名前付きの定理において，セミコロンがそれに続くタクティクのブロックを ``two_le`` を適用した結果のゴールそれぞれに適用していることを覚えておいてください．
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ここからが本番です．素数の集合 ``s`` が与えられたときに，その集合から3が存在する場合に3を取り除いたものについて話す必要があります．関数 ``Finset.erase`` がこれを実現します．
EXAMPLES: -/
-- QUOTE:
