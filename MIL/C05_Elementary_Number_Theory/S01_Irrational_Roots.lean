...
OMIT. -/
/- TEXT:
素因数分解の一意性とは，0以外の自然数は素数の積として一意に書けるというものです．Mathlibにはこの定理が正式に含まれており， ``Nat.factors`` という関数で表現され，与えられた数の素因数を小さい順に並べたリストを返します．Mathlibは ``Nat.factors n`` のすべての要素が素数であること，0より大きい ``n`` はその因数の積に等しいこと，そして ``n`` が他の素数のリストの積に等しい場合，そのリストは ``Nat.factors n`` の結果を順列に並べ替えたものであることを証明します．
EXAMPLES: -/
-- QUOTE:
#check Nat.factors
...
OMIT: -/
/- TEXT:
ここまで行ってきた証明について改善するポイントがいくつかあります．まずはじめに，2の平方根が無理数であることの証明は，2の平方根について語るべきであり，それは実数または複素数の要素として理解されているもののことです．そして2の平方根が無理数であることを証明することは有理数について語るべきであり，すなわち2の平方根に等しい有理数は無いということを示すはずです．さらにこの節の定理を整数に拡張する必要があります．もし2の平方根を2つの整数の商として書くことができれば，自然数の商としても書くことができるのは数学的に明らかですが，これを正式に証明するには多少の努力が必要です．
