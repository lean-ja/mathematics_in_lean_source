...
OMIT: -/
/- TEXT:
古代ギリシャの時代から知られていた事実から初めていきましょう．すなわち2の平方根が無理数であるということです．もしここでそうでないと仮定すると， :math:`\sqrt{2} = a / b` のようにとある既約分数として書くことができます．両辺を2乗すると :math:`a^2 = 2 b^2` となります．これは :math:`a` が偶数であることを意味します．そこで :math:`a = 2c` と書くと :math:`4c^2 = 2 b^2` となり整理して :math:`b^2 = 2 c^2` を得ます．これは :math:`b` も偶数であることを意味し， :math:`a / b` が既約分数として仮定に矛盾します．
...
OMIT. -/
/- TEXT:
:math:`a / b` が既約分数であるということは， :math:`a` と :math:`b` は共通の因数がない，つまり *互いに素* であることを意味します．Mathlibは述語 ``Nat.coprime m n`` を ``Nat.gcd m n = 1`` と定義しています．Leanの無名の射影による記法を使うと， ``s`` と ``t`` が ``Nat`` 型の式であれば， ``Nat.coprime s t`` の代わりに ``s.coprime t`` と書くことができます．これは ``Nat.gcd`` についても同様です．いつものように，Leanはしばしば ``Nat.coptime`` の定義を必要に応じて自動的に展開しますが， ``Nat.coprime`` 識別子を用いて手動で書き換えや簡約することもできます． ``norm_num`` タクティクは具体的な値を計算するにあたって十分な賢さを備えています．
EXAMPLES: -/
-- QUOTE:
#print Nat.coprime
...
OMIT: -/
/- TEXT:
関数 ``gcd`` にはすでに :numref:`more_on_order_and_divisibility` で説明しました． ``gcd`` には整数用のバージョンも存在します．このような異なる数のシステム間の関係については後述します．また ``gcd`` 関数や ``Prime`` ， ``coprime`` にはさらに一般的な概念も存在し，一般的な代数構造で意味を持ちます．Leanがこの一般性をどのように管理しているかについては次の章で説明します．それまでにこの節では自然数に限定して説明しましょう．
...
OMIT: -/
/- TEXT:
自然数では，素数は自明ではない因数の積として書けないという性質を持っています．より広い数学的な文脈では，この性質を持つ環の元は *既約元* と呼ばれます．環のある元が *素数* であるとは，この元が積を割れる時，必ずその積の因数のいずれかを割ることができることを指します．この2つの概念は一致し，自然数の重要な性質として定理 ``Nat.Prime.dvd_mul`` に集約されます．
...
OMIT. -/
/- TEXT:
この事実は上記で行った議論の重要な性質を確立することができます．すなわちある数の2乗が偶数であれば，その数も偶数であるということです．Mathlibは ``Data.Nat.Parity`` にて述語 ``Even`` を定義していますが，後ほど説明しますがここでは ``m`` が偶数であることを表現するために ``2 ∣m`` を使用します．
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
本書を読み進めるにあたって，ライブラリから必要な事実や定理を見つける技量が必要になってきます．もし名前の接頭辞が推測でき，関連するライブラリをimportしているのであれば，タブ補完（ ``Ctrl+タブ`` を使うこともあります）を使って探しているものを見つけることができることを覚えておいてください．任意の識別子を ``Ctrl+クリック`` することでその識別子が定義されているファイルにジャンプすることができます．これでその識別子の定義とその近くにある定理を見ることが可能です．また `LeanのコミュニティのWEBページ <https://leanprover-community.github.io/>`_ にある検索エンジンを使うこともできますし，もし他の方法がうまくいかなかったら遠慮なく `Zulip <https://leanprover.zulipchat.com/>`_ で質問してください．
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
2の平方根が無理数であることの証明の確信は次の定理にあります．この証明について ``even_of_even_sqr`` と定理 ``Nat.dvd_gcd`` を使ってby以降を埋めてみましょう．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
素因数分解の一意性とは，0以外の自然数は素数の積として一意に書けるというものです．Mathlibにはこの定理が正式にふくまれており， ``Nat.factors`` という関数で表現され，与えられた数の素因数を小さい順に並べたリストを返します．ライブラリは ``Nat.factors n`` のすべての要素が素数であること，0より大きい ``n`` はその因数の積に等しいこと，そして ``n`` が他の素数のリストの積に等しい場合，そのリストは ``Nat.factors n`` の結果を順列に並べ替えたものであることを証明します．
EXAMPLES: -/
-- QUOTE:
#check Nat.factors
...
OMIT: -/
/- TEXT:
次の例は簡約化が ``n^2 ≠ 0`` を ``n ≠ 0`` に置き換えられるくらい賢いことを示しています． ``simpa`` タクティクはただ単に ``simp`` タクティクのあとに ``assumption`` タクティクを呼んでいるだけです．
...
OMIT: -/
/- TEXT:
``Nat.count_factors_mul_of_pos`` を ``r * n^k`` と一緒に使うには， ``r`` が正であることを知っている必要があります．しかし ``r`` が0の場合，以下の定理は自明であり，簡約化で簡単に証明できます．そこで証明は場合分けして行うことにします．一つは ``r`` を ``0`` に置き換えるもので，もう一つは ``r`` を ``r.succ`` に置き換えるものです．後者の場合，定理 ``r.succ_ne_zero`` を使うことができ， ``r.succ ≠ 0`` が成立します．
...
OMIT: -/
/- TEXT:
以下の証明の書けている部分を埋めてみましょう．証明の一番最後は ``Nat.dvd_sub'`` と ``Nat.dvd_mul_right`` で完成させることができます．
BOTH: -/
-- QUOTE:
...
OMIT: -/
/- TEXT:
ここまでの行ってきた証明について改善するポイントがいくつかあります．まずはじめに，2の平方根が無理数であることの証明は，2の平方根について語るべきであり，それは実数または複素数の要素として理解されているもののことです．そして2の平方根が無理数であることを証明することは有理数について語るべきであり，すなわち2の平方根に等しい有理数は無いということを示すはずです．さらにこの節の定理を整数に拡張する必要があります．もし2の平方根を2つの整数の商として書くことができれば，自然数の商としても書くことができるのは数学的に明らかですが，これを正式に証明するには多少の努力が必要です．
