...
OMIT: -/
/- TEXT:
関数 ``gcd`` にはすでに :numref:`more_on_order_and_divisibility` で説明しました． ``gcd`` には整数用のバージョンも存在します．このような異なる数体系間の関係については後述します．また ``gcd`` 関数や ``Prime`` ， ``coprime`` にはさらに一般的な概念も存在し，一般的な代数構造で意味を持ちます．Leanがこの一般性をどのように管理しているかについては次の章で説明します．それまでにこの節では自然数に限定して説明しましょう．
