...
OMIT. -/
/- TEXT:
そのため，Leanで ``s / t`` という式を読むときに ``t`` が意味のある入力値であることを仮定すべきではありません．もしその仮定が必要である場合は，定理の内容を制限して，それが意味のある値であることを保証することができます．例えば，定理 ``div_mul_cancel`` は適切な代数構造の ``x`` と ``y`` に対して ``x ≠ 0 → x / y * y = x`` を保証するものです．
...
OMIT. -/
/- TEXT:
本節を終えるにあたって，ある集合からその冪集合への全射が存在しないというカントールの有名な定理を型理論的に述べましょう．この証明を理解した上で，欠けている2行を埋めてみてください．
TEXT. -/
-- BOTH:
section
variable {α : Type*}
