...
OMIT. -/
/- TEXT:
関数 ``f : α → β`` の逆関数を定義するにあたって，新しい概念を2つ用意しましょう．まず，型が空であるかもしれないという事実に対処する必要があります． ``f`` の逆関数を定義するにあたって，ある ``y`` に対して ``f x = y`` を満たすような ``x`` が無い場合， ``α`` 型のデフォルト値を割り当てたいところです．``[Inhabited α]`` を ``variable`` として追加することは ``α`` 型がまさにこのような要素を持つことと同義であり，この要素は ``default`` と表記されます．次に， ``f x = y`` を満たすような ``x`` が複数存在する場合，逆関数はそのうちの1つだけを *選択* する必要があります．これには *選択公理(axiom of choice)* の力を借りる必要があります．Leanでは様々な方法で選択公理にアクセスすることができます．1つの便利な方法は，以下に示す ``Classical.choose`` 関数を使用することです．
TEXT. -/
-- BOTH:
section
-- QUOTE:
