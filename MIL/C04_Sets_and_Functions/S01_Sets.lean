...
OMIT. -/
/- TEXT:
集合について証明する一つの方法は， ``rw`` や ``simp`` を使って定義を展開することです．以下の2つ目の例では， ``simp only`` を使うことで ``simp`` がデータベース全体ではなく，指定した等式のリストだけを使うようにしています． ``rw`` とは異なり， ``simp`` は全称量化子や存在量化子の中でも単純化を行うことができます．以下の証明を一行ずつたどれば，これらのタクティクの効果を見ることができます．
TEXT. -/
-- BOTH:
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
Leanの独特な入力に対する処理によって， ``theorem foo`` を ``example`` に置き換えると最初の例は失敗してしまいます．これは定義に基づく簡約を過度に信頼することの落とし穴を示しています．定義に基づく簡約は便利なことも多いですが，手動で定義を展開しなければならないこともあります．
...
OMIT. -/
/- TEXT:
Leanで集合を表現するとき，その裏で何が行われているのか説明しましょう．型理論において，ある型 ``α`` の *性質* や *述語* は単なる関数 ``P : α → Prop`` です．これは理にかなっています: ``a : α`` が与えられた時， ``P a`` はまさに ``a`` に対して ``P`` が成り立っているという命題に他ならないからです．Mathlibでは ``Set α`` は ``α → Prop`` と定義され， ``x ∈ s`` は ``s x`` と定義されます．言い換えれば，集合は性質であり，対象として扱われます．
