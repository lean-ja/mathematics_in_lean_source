...
OMIT. -/
/- TEXT:
:numref:`proving_identities_in_algebraic_structures` では，実数上で成り立つ多くの一般的な恒等式が，可換環のようなより一般的な代数的構造のクラスでも成り立つことを見ました．等式だけでなく，代数的構造を記述するために必要な公理はなんでも使うことができます．例えば *半順序* は実数上の ``≤`` のような反射的で推移的な二項関係を持つ集合で構成されます．Leanはこの半順序を備えています:
TEXT. -/
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
ここでは任意の型としてMathlibの慣例に則り ``α`` や ``β`` ， ``γ`` （ ``\a`` ， ``\b`` ， ``\g`` と入力します）等を使用しています．ライブラリでは環や群のような代数的構造を表す際にしばしば ``R`` や ``G`` の文字を使いますが，一般的な型，特に構造に関連するものがほとんど無い場合にはギリシャ文字が使われます．
...
OMIT. -/
/- TEXT:
任意の半順序 ``≤`` に関連して，実数上で ``<`` のような働きをする *狭義半順序* ``<`` も存在します．この関係において ``x`` が ``y`` より小さいというのは ``y`` 以下でかつ ``y`` と等しくないということと同じです．
TEXT. -/
-- QUOTE:
#check x < y
