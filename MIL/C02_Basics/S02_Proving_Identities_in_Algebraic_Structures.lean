...
OMIT. -/
/- TEXT:
数学的に，環は対象の集まりである :math:`R` と演算子 :math:`+` :math:`\times`，定数 :math:`0`，:math:`1`，そして演算子 :math:`x \mapsto -x` から構成され，以下を満たします:
...
OMIT. -/
/- TEXT:
Leanでは対象のあつまりは *型* として表現され，環は ``R`` と表記します．環の公理は以下のようになります．
...
OMIT. -/
/- TEXT:
上記の1行目に出てきた角括弧については後ほど学びますが，今のところはこの書き方で型 ``R`` に対して環の構造が与えられるようにする定義だと思ってもらえれば十分です．これによりLeanは ``R`` の元について環の一般的な記法を使うことが出来るようにし，ライブラリ中の環についての定理も使えるようになります．
...
OMIT. -/
/- TEXT:
これらの定理の中にはなじみ深い名前のものもあるでしょう，それもそのはずで，これらはまさに前章で実数について計算したものと同じだからです．Leanは実際の自然数や整数のような数学構造について証明をするだけでなく，環のような公理的に特徴づけられた抽象構造の証明にも長けています．さらに，Leanは抽象と具象の構造どちらに対しても *一般的な推論* のサポートを備えており，適切なインスタンスを認識できるように調節できます．そのため環についての定理を整数 ``Z`` や有理数 ``Q`` ，複素数 ``C`` のような具体的な環に適用することができます．またさらに任意の順序環や体などの拡張された環構造のインスタンスに適用することもできます．
...
OMIT. -/
/- TEXT:
しかし，実数の重要な性質がすべて任意の環で成り立つわけではありません．例えば実数の乗算は可換ですが，これは一般の環では成り立ちません．線形代数の講義を受けたことがある人なら，実数の :math:`n` 行列が :math:`n` × :math:`n` の環を形成し，その環では通常可換性が成り立たないことがわかります．もし ``R`` を *可換* 環であると宣言すれば，実際に ``ℝ`` を ``R`` に置き換えても，前節の定理はすべて成り立ち続ける．
TEXT. -/
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
他のすべての証明が変更されずに通過することを確認するのは読者のために残しましょう．``by ring`` や ``by linarith`` や ``by sorry`` のように証明が短い場合，証明は ``by`` と同じ行に書くのが一般的です（また許可されています）．良い証明文の書き方は，簡潔さと読みやすさのバランスを取ることです．
...
OMIT. -/
/- TEXT:
このセクションの目的は，前回のセクションで身につけたスキルを強化し，環についての公理的推論に応用することです．上に挙げた公理から始め，それを使って他の事実を導きます．本書で証明する事実のほとんどはすでにMathlibにあります．Mathlibの内容と命名規則を学ぶのに役立つように，証明するものに同じ名前を付けます．
...
OMIT. -/
/- TEXT:
Leanは一般的なプログラミング言語で使われるものと似たようなコードの組織化のメカニズムを提供します．定義や定理 ``foo`` が *名前空間* ``bar`` に導入されると，その完全な名前は ``bar.foo`` となります．コマンド ``open bar`` で名前空間を *開く* ことで，より短い名前 ``foo`` を使うことができます．名前の衝突によるエラーを避けるために，次の例ではライブラリの定理を ``MyRing.`` という新しい名前空間に置きます．
...
OMIT. -/
/- TEXT:
このように名前空間を分けることの正味の効果は，ライブラリにある定理を一時的に再現し，その後はライブラリのバージョンを使い続けることができるということです．しかし，ごまかしは禁物です！この後の練習問題では，このセクションで先に証明した環に関する一般的な事実だけを使うように注意しましょう．
...
OMIT. -/
/- TEXT:
(注意深く見ていれば，``(R : Type*)`` の丸括弧を ``{R : Type*}`` の中括弧に変更したことに気づいたかもしれません．これは ``R`` が *暗黙の引数* であることを宣言しています．これが何を意味するかは後ほど説明しますが，ひとまずは気にしないでください)
...
OMIT. -/
/- TEXT:
ここで中括弧の使い方について説明しましょう．例えば証明の文脈中に ``a`` ， ``b`` ， ``c`` ，仮説 ``h : a + b = a + c`` があり，結論 ``b = c`` を導きたいとしましょう．Leanでは，定理を ``a`` や ``b`` 等の対象に適用するのと同じように仮説や事実に適用することができるため，``add_left_cancel a b c h`` が ``b = c`` の証明だと思われるかもしれません．しかし， ``a`` ， ``b`` ， ``c`` を明示的に書くことは冗長であることに注意してください．なぜなら，仮説 ``h`` にはこれらの対象を含んでいるため，この仮説のみでどういう対象を想定しているかが明確だからです．今回の場合，数文字書くかどうか程度ですので大した負担にはなりませんが，``add_left_cancel`` をさらに複雑な式に適用していく場合には面倒になることでしょう．このような場合のためにLeanでは引数を *暗黙的* なものとしてマークすることができます．つまり，引数は省略され，その引数を用いている別の引数や仮説などの他の要素から推測されるようになっているのです．これがまさに ``{a b c : R}`` での中括弧です．以上より，上記の定理の型から考えると正しい式は ``add_left_cancel h`` とシンプルなものとなります．
...
ここでは新しいトリックを使っています！上記の証明をステップごとにinfoviewを見ていけば，この証明で何が起こっているかわかるでしょう．この ``have`` タクティクは新しいゴール ``a * 0 + a * 0 = a * 0 + 0`` を導入しています．この次の行でインデントが1段深くなっているのは，Leanがこの新しいゴールの証明をするためのタクティクによるブロックが続くことを期待していることを示しています．つまりこのインデントによってモジュール化した証明，すなわち ``have`` で導入されたインデントされたサブ証明のゴールというスタイルを導入しているのです．このインデントの後，証明はもとのゴールの証明へと戻りますが，先程証明した新しい仮説 ``h`` が追加され，これを自由に使うことができます．この新しい仮説の追加をした時点で，ゴールはまさに ``add_left_cancel h`` となります．
...
OMIT. -/
/- TEXT:
上記の証明の終わりにおいて ``rw`` を用いていますが，これの代わりに ``apply add_left_cancel h`` もしくは ``exact add_left_cancel h`` で証明を終えることもできます．``exact`` タクティクは新しいゴールを作らずに，現在のゴールを完全に証明できている項を引数にとります．これに対し ``apply`` タクティクはもう少し広く，引数は完全な証明である必要はありません．完全な証明ではない場合，足りない部分はLeanが自動的に推測するか，証明すべき新たなゴールとなります．``exact`` は厳密には ``apply`` よりも弱いため技術的に冗長なタクティクですが，これにより証明のスクリプトが読む人にとって若干わかりやすくなり，ライブラリが進化したときのメンテナスも容易になります．
...
OMIT. -/
/- TEXT:
Leanはどんな環でも ``1 + 1 = 2`` が成り立つことを知っています．そしてさらに少し努力すれば，この事実を使って最後のセクションの定理 ``two_mul`` を証明することができます:
TEXT. -/
-- QUOTE:
-- BOTH:
...
これらの補題を明示的に呼び出すのは面倒であるため，Mathlibは上記の補題のほとんどすべての用途をカバーするような `ring` と同じようなタクティクを提供しています:`group` は非可換乗法群， `abel` は加法群，そして `noncomm_ring` は非可換環です．代数的な構造が `Ring` や `CommRing` と呼ばれている一方で，タクティクが `noncomm_ring` と `ring` と名付けられているのは奇妙に感じられるかもしれません．これには歴史的な理由もありますが，可換環を扱う場合のほうが多いため，それに合わせて便宜上短い名前を可換環につけているという理由もあります．
TEXT. -/
