...
OMIT. -/
/- TEXT:
* :math:`R` は :math:`+` に関して :math:`0` を加法の単位元，減算を逆元とする **アーベル群** （abelian group）．
...
OMIT. -/
/- TEXT:
Leanは一般的なプログラミング言語で使われるものと同様の **名前空間** （namespace）によるコードの組織化のメカニズムを提供します．例えば定義や定理 ``foo`` が名前空間 ``bar`` に導入されると，その完全な名前は ``bar.foo`` となります．コマンド ``open bar`` で名前空間を **開く** ことができ，より短い名前 ``foo`` を使えるようになります．名前の衝突によるエラーを避けるために，次の例ではライブラリの定理を ``MyRing`` という新しい名前空間に置きます．
...
OMIT. -/
/- TEXT:
(注意深い読者なら，``(R : Type*)`` の丸括弧を ``{R : Type*}`` の中括弧に変更したことに気づかれたかもしれません．これは ``R`` が **暗黙の引数** （implicit argument）であることを宣言しています．これが何を意味するかは後ほど説明しますが，ひとまずは気にしないでください)
...
本節を締めくくるにあたって，上で証明した加算と減算に関する事実の中には，環の公理のすべてを必要とするわけではないものや，加算の可換性さえも必要としないものがあることについて見ていきましょう．この環よりも弱い概念である **群** （group）は次のように公理化できます:
TEXT. -/
section
-- QUOTE:
