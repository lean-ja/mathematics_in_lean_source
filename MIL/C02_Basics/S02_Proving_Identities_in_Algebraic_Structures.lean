...
OMIT. -/
/- TEXT:
しかし，実数の重要な性質がすべて任意の環で成り立つわけではありません．例えば実数の乗算は可換ですが，これは一般の環では成り立ちません．線形代数の講義を受けたことがある人なら，実数の :math:`n` 行列が :math:`n` × :math:`n` の環を形成し，その環では通常可換性が成り立たないことがわかります．もし ``R`` を *可換* 環であると宣言すれば，実際に ``ℝ`` を ``R`` に置き換えても，前節の定理はすべて成り立ちます．
TEXT. -/
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
この節の目的は，前節で身につけたスキルを強化し，環についての公理的推論に応用することです．上に挙げた公理から始め，それを使って他の事実を導きます．本書で証明する事実のほとんどはすでにMathlibにあります．Mathlibの内容と命名規則を学ぶのに役立つように，証明するものに同じ名前を付けます．
...
OMIT. -/
/- TEXT:
このように名前空間を分けることの正味の効果は，ライブラリにある定理を一時的に再現し，その後はライブラリのバージョンを使い続けることができるということです．しかし，ごまかしは禁物です！この後の練習問題では，この節で先に証明した環に関する一般的な事実だけを使うように注意しましょう．
...
OMIT. -/
/- TEXT:
上記の証明の終わりにおいて ``rw`` を用いていますが，これの代わりに ``apply add_left_cancel h`` もしくは ``exact add_left_cancel h`` で証明を終えることもできます．``exact`` タクティクは新しいゴールを作らずに，現在のゴールを完全に証明できている項を引数にとります．これに対し ``apply`` タクティクはもう少し広く，引数は完全な証明である必要はありません．完全な証明ではない場合，足りない部分はLeanが自動的に推測するか，証明すべき新たなゴールとなります．``exact`` は厳密には ``apply`` よりも弱いため技術的に冗長なタクティクですが，これにより証明のスクリプトが読む人にとって若干わかりやすくなり，ライブラリが進化したときのメンテナンスも容易になります．
