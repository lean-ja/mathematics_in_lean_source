...
OMIT. -/
/- TEXT:
ここで ``min`` を引数 ``a`` と ``b`` のペアに適用するにあたり ``min (a b)`` ではなく ``min a b`` としている点に注意してください．形式的には ``min`` は ``ℝ → ℝ → ℝ`` 型の関数です．このように複数の矢印を用いた型を記述する場合，この矢印は慣例的に右結合として ``ℝ → (ℝ → ℝ)`` と解釈されます．この結果， ``a`` と ``b`` が ``ℝ`` 型の場合， ``min a`` は ``ℝ → ℝ`` 型となり， ``min a b`` は ``ℝ`` 型を持つため ``min`` は期待通り2引数の関数として振る舞います．このように複数の引数を取り扱うことは，論理学者のHaskell Curryにちなんで *カリー化* として知られています．
...
OMIT. -/
/- TEXT:
またLeanの演算子の優先順位についても慣れが必要です．関数適用は中置演算子より強く束縛されるため， ``min a b + c`` という式は ``(min a b) + c`` と解釈されます．これらの慣例は時間が経てば自然と身につくでしょう．
...
OMIT. -/
/- TEXT:
``le_antisymm`` を使用することで，2つの実数が等しいのはそれぞれが他方より小さいか等しい場合であることを示すことができます．これと上記の事実を使えば ``min`` が可換であることを示すことができます:
TEXT. -/
-- QUOTE:
...
ここでは異なるゴールの証明を区切るためにピリオドを用いています．しかしこれらのピリオドの使い方は一貫していません:外側のレベルでは両方のゴールにピリオドとインデントを用いていますが，入れ子になった証明ではゴールが一つになるまでだけピリオドを使用しています．これらの書き方も合理的で有用です．また ``show`` タクティクを使って証明を構成し，各ブロックで何を証明したいかを明示的にしています．``show`` コマンドを使わなくても証明は機能しますが，これにより証明が読みやすくなり，保守もしやすくなります．
...
OMIT. -/
/- TEXT:
この証明では同じようなことを繰り返していることが気になるかもしれません．あとで学ぶことになるスキルを予感させるために，繰り返しを避ける一つの方法として局所的な補題を述べてからそれを利用する方法を記します:
TEXT. -/
-- QUOTE:
example : min a b = min b a := by
...
OMIT. -/
/- TEXT:
全称記号については :numref:`implication_and_the_universal_quantifier` で詳しく述べますが，今の時点では仮説 ``h`` は任意の ``x`` と ``y`` に対して目的の不等式が成り立っていて，``intro`` タクティクは任意の ``x`` と ``y`` を導入して結論を導いていると理解できていれば十分です． ``le_antisymm`` のあとの一番始めの ``apply`` は暗黙的に ``h a b`` を用いている一方で，2つめでは ``h b a`` を使っています．
...
OMIT. -/
/- TEXT:
別解として ``repeat`` タクティクを用いる方法もあります．これはタクティク（や証明のブロック）をできるだけ繰り返し適用するタクティクです．
TEXT. -/
-- QUOTE:
example : min a b = min b a := by
  apply le_antisymm
...
OMIT. -/
/- TEXT:
乗法が加法に分配されるように ``min`` 関数が ``max`` に分配され，その逆も成り立つというのは興味深い事実です．言い換えれば，実数上において ``min a (max b c) ≤ max (min a b) (min a c)`` という式の ``max`` と ``min`` を入れ替えた同様の対応する式があるということです．しかし次の節では， ``≤`` の推移性と反射性，そして上記で示した ``min`` と ``max`` の特徴的な性質からはこれが *成り立たない* ことを見ていきます．その代わりにこの性質は実数上の ``≤`` が ``∀ x y, x ≤ y ∨ y ≤ x`` という関係を満たす *全順序* であるという事実を利用する必要があります．ここで論理和の記号 ``∨`` は「または」を表します．最初のケースでは ``min x y = x`` となり，2番めのケースでは ``min x y = y`` となります．大文字と小文字を区別する方法は :numref:`disjunction` で学びますが，現時点では大文字と小文字を区別する必要のない例で説明します．
