...
OMIT. -/
/- TEXT:
詳しくは :numref:`implication_and_the_universal_quantifier` で説明しますが，命題 ``le_trans`` について ``→`` は右結合であるためこの命題は ``a ≤ b → (b ≤ c → a ≤ c)`` と解釈されます．ライブラリの設計者は ``le_trans`` の引数 ``a`` ， ``b`` ， ``c`` を暗黙に設定したため，Leanはこれらの引数を（このあと議論するように，実際に要求する場合を覗いて）明示的には利用者に提供 *しません* ．むしろこれらの引数が使用されている文脈から推測することを期待します．例えば，仮定 ``h : a ≤ b`` と ``h' : b ≤ c`` が文脈にある場合，これらは以下のように働きます:
TEXT. -/
section
-- QUOTE:
