...
``apply`` タクティクは一般的な命題や含意の証明を受け取り，結論を現在のゴールにマッチさせようとし，もし仮定が残る場合はそれを新しいゴールとします．もし与えられた証明がゴールに正確に一致する場合（ **定義上** （definitional）等しい変形が残っている場合も含みます），``apply``　の代わりに ``exact`` タクティクを使うことができます．したがって次のように書けます:
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
``linarith`` タクティクは **線形算術** （linear arithmetic）を扱えるように設計されています．
TEXT. -/
section
...
OMIT. -/
/- TEXT:
上記の定理の中で， ``exp_le_exp`` と ``exp_lt_exp`` ， ``log_le_log`` は必要十分条件を意味する **双方向の含意** （bi-implication）を用いています．（VSCodeでは ``\lr`` か ``\iff`` で入力することができます）．この論理結合子については次の章で詳しく説明することとします．このような定理は ``rw`` と一緒に使うことで，ゴールをそれと等価なものに書き換えることができます:
TEXT. -/
-- QUOTE:
