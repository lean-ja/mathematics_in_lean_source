...
OMIT. -/
/- TEXT:
書き換えは等式を証明するのには最適ですが，等式以外の定理についてはどうでしょうか？
例えば， :math:`b \le c` であるときに :math:`a + e^b \le a + e^c` が成り立つというような不等式はどのように証明するのでしょうか？
定理が何かしらの引数や仮説に適用でき， ``apply`` や ``exact`` タクティクを用いてゴールを解決するために使うことができることはこれまでに見てきました．
このセクションでは，これらのツールをうまく使っていきましょう．
...
OMIT. -/
/- TEXT:
詳しくは :numref:`implication_and_the_universal_quantifier` で説明しますが，
命題 ``le_trans`` について ``→`` は右結合であるためこの命題は ``a ≤ b → (b ≤ c → a ≤ c)`` と解釈されます．
ライブラリの設計者は ``le_trans`` の引数 ``a`` ， ``b`` ， ``c`` を暗黙に設定したため，
Leanはこれらの引数を（このあと議論するように，実際に要求する場合を覗いて）明示的には利用者に提供 *しません* ．
むしろこれらの引数が使用されている文脈から推測することを期待します．
例えば，仮説 ``h : a ≤ b`` と ``h' : b ≤ c`` が文脈にある場合，これらは以下のように働きます:
TEXT. -/
section
-- QUOTE:
...
``apply`` タクティクは一般的な命題や含意の証明を受け取り，結論を現在のゴールにマッチさせようとし，
もし仮定が残る場合はそれを新しいゴールとします．
もし与えられた証明がゴールに正確に一致する場合（ *定義的に* 等しい変形が残っている場合も含みます），
``apply``　の代わりに ``exact`` タクティクを使うことができます．
これらのことから以下のすべての証明が機能します．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
最初の例では， ``le_trans`` を適用することで2つのゴールが作成され，それぞれの証明がどこから始まるかをピリオドで表しています．
ここでピリオドは使用しなくても証明は通りますが，ピリオドを使用することでゴールに *焦点があてられます* :
ピリオドが示すブロック内ではゴールは一つしかinfoviewに表示されず，かつこのゴールはブロックが終わるまでに完了しなければなりません．
ここでは最初のブロックは次の行から始まる別のピリオドによって終了しています．
この例に対し，インデントすることなく証明することも可能です．
4つ目と最後の例では完全にタクティクモードを避けています:
``le_trans h₀ h₁`` と ``le_refl x`` がまさに証明に必要な項です．
...
OMIT. -/
/- TEXT:
文脈中にある等式と不等式に加えて， ``linarith`` は引数として渡した追加の不等式を使用します．
次の例ではこのあとすぐ説明するように， ``exp_le_exp.mpr h'`` が ``exp b ≤ exp c`` の証明です．
ここでLeanにおいて引数 ``x`` に関数 ``f`` を適用した結果を表すために ``f x`` と記載しますが，
これは引数 ``x`` に事実や定理 ``h`` を適用した結果を表すために ``h x`` と書くのと全く同じです．
他の一般的なプログラミング言語のように関数適用に括弧は不要ですが， ``f (x + y)`` のように複合的な引数の場合には必要となります．
この場合括弧がないと， ``f x + y`` は ``(f x) + y`` と解釈されます．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
上記の定理の中で， ``exp_le_exp`` と ``exp_lt_exp`` ， ``log_le_log`` は必要十分条件を意味する *双方向の含意* を用いています．
（VSCodeでは ``\lr`` か ``\iff`` で入力することができます）
この接続詞については次の章で詳しく説明することとします．
このような定理は ``rw`` と一緒に使うことで，ゴールをそれと等価なものに書き換えることができます:
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
しかし，本節では ``h : A ↔ B`` がこのような同値である場合， ``h.mp`` を順方向である ``A → B`` として，
``h.mpr`` を逆方向である ``B → A`` として成立させているという事実の方を使っていきます．
ここで ``mp`` は"modus ponens"を表し， ``mpr`` は"modus ponens reverse"を表しています．
また ``h.mp`` と ``h.mpr`` はそれぞれ ``h.1`` と ``h.2`` を代わりに使うこともできます．
以上から次の証明が成り立ちます:
TEXT. -/
-- QUOTE:
...
/- TEXT:
次の例は読者自身で試してみてください．
真ん中の例では ``norm_num`` というタクティクが具体的な数値についてのゴールを解決するために使えることを示しています．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ここまで見てきた例題から，形式化において重要なことは読者がライブラリから必要な定理を見つけられるようになることであるのは明らかでしょう．
これに当たって，取りうる手段はたくさんあります:
...
OMIT. -/
/- TEXT:
* Mathlibの `GitHubのリポジトリ <https://github.com/leanprover-community/mathlib4>`_ をブラウザで見ることができます．
...
OMIT. -/
/- TEXT:
* Mathlibの `webページ <https://leanprover-community.github.io/mathlib4_docs/>`_ にあるAPIのドキュメントから探すこともできます．
...
OMIT. -/
/- TEXT:
* Mathlibの定理はある程度命名規則が決まっているため，
  ある程度定理名を推理して入力した上でCtrl+スペースキー（MacではCmd+スペース）によってエディタで定理名を補完することも可能です．
  Leanでは， ``A_of_B_of_C`` という名前の定理は ``B`` と ``C`` という仮説から ``A`` という形の何かの証明に当てられ，
  そしてこの ``A`` ， ``B`` ， ``C`` はゴールを読み上げたときの文におおよそ近いことが多いです．
  つまり，例えば ``x + y ≤ ...`` というような形の定理はおそらく ``add_le`` で始まる可能性が高いのです．
  そこで ``add_le`` まで入力し，Ctrl+スペースを押せば有益な選択肢が表示されることでしょう．
...
OMIT. -/
/- TEXT:
* VSCode上で既存の定理名を右クリックして出てくるオプション中に，
  その定理が定義されているファイル中の位置に飛ぶ選択肢があるため，
  それを利用してその定理の近くにある類似の定理を探すことができます．
...
OMIT. -/
/- TEXT:
この例で ``exact`` の行を削除し，その前の行のコメントアウトを外して ``apply?`` の挙動を確認してみてください．
この便利なツールを使って次の例でも必要な定理が見つかるかどうか試してみましょう:
TEXT. -/
-- QUOTE:
...
この例では注目すべき点がいくつもあります．
まず， ``s ≥ t`` は定義上 ``t ≤ s`` と等価です．
原理的にこの2つは互換性があるべきでしょう．
しかし，Leanの自動証明の機構の中にはこの同値性を認識しない場合もあるため，Mathlibは ``≥`` よりも ``≤`` をより優先する傾向があります．
第二に， ``ring`` タクティクをかなり多用しています．
これはとても時間の節約になります！
そして最後に，2番めの ``calc`` の証明の2行目で ``by exact add_le_add (le_refl _) h`` と書く代わりに，
証明項 ``add_le_add (le_refl _) h`` をそのまま書けばよいことに注意してください．
...
OMIT. -/
/- TEXT:
実際，上記の証明で唯一ひらめきを要するのは仮説 ``h`` を見出すことだけです．
それさえわかれば，2つ目の計算は線形演算だけで，これは ``linarith`` で処理できます:
TEXT. -/
-- QUOTE:
example : 2 * a * b ≤ a ^ 2 + b ^ 2 := by
  have h : 0 ≤ a ^ 2 - 2 * a * b + b ^ 2
...
/- TEXT:
なんて素晴らしいのでしょうか！
これらのアイデアをもとに次の定理を証明してみてください．
この定理では ``abs_le'.mpr`` を使うことができます．
TEXT. -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
これを自力で解けたのならおめでとうございます！
あなたは形式化の達人への道を順調に歩めています．
TEXT. -/
