...
/- TEXT:
Leanにおいて，定理を述べるということは，その定理を証明するというゴールを述べることと同等です．Leanには，ゴールの左辺をそれと等しい右辺に置き換えるタクティックである ``rw`` が存在します．``a`` ， ``b``， ``c`` が実数の時， ``mul_assoc a b c`` は ``a * b * c = a * (b * c)`` という恒等式であり， ``mul_comm a b`` は ``a * b = b * a`` という恒等式を表しています．Lean には自動化の仕組みがあるので，こうして明示的に事実を参照する必要はないのですが，説明のためには ``rw`` が便利です．Leanにおいて，掛け算は左側から計算されます．よって左側にある ``mul_assoc`` は ``(a * b) * c`` と書くことも可能です．しかしながら，一般的にはLeanの表記規則に倣い，Leanが括弧を省略する場合は括弧を省略するのが良いとされるスタイルです．
``rw`` を試してみましょう！
...
/- TEXT:
この証明が ``by`` で始まっていないことに注意してください．``calc`` で始まる式は証明項です．``calc`` 式はタクティクを使用した証明の中で使うこともできますが，Leanはそれを，結果として得られる証明項を使ってゴールを証明するという指示として解釈します．``calc`` は書く際に注文の多いコマンドです．下線の引き方と正当化は上記の形式でなければなりません．インデントを使って，タクティックのブロックや ``calc`` ブロックの始まりと終わりを決定しています．上の証明のインデントを変えてみてどうなるか見てみましょう．
``calc`` による証明の書き方の１つの方法として，まず各式変形の証明を ``sorry`` タクティクで置いて式変形の概要を書き，これをLeanが受け入れることを確認してから ``sorry`` としていた各式変形を証明していく，というものがあります．
TEXT. -/
