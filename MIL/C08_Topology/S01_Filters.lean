...
OMIT. -/
/- TEXT:
* *やがて(eventually)起こりうること* ，これは十分大きい ``n : ℕ`` ，もしくはある点 ``x`` の十分近傍について，あるいは十分近い点のペアや測度論の意味でのほぼすべての場所で起こることを含みます．また，それと対をなすようにフィルタは任意の大きさの点 ``n`` や，任意の近傍の点などについて， *頻繁に起こること* を表現することもできます．
...
OMIT. -/
/- TEXT:
最初の条件は ``X`` のすべての要素の集合は ``F.sets`` に属するというものです．2つ目の条件は ``U`` が ``F.sets`` に属する場合， ``U`` を含むものはすべて ``F.sets`` に属するということです．3つ目の条件は， ``F.sets`` は有限な共通集合で閉じているということです．Mathlibでは，フィルタ ``F`` は ``F.sets`` とその3つのプロパティをまとめた構造として定義されていますが，プロパティはデータを追加で持つことはないため， ``F`` と ``F.sets`` の区別を曖昧にするのに便利です．したがって， ``U ∈ F`` を ``U ∈ F`` と定義します．これが， ``U ∈ F`` に言及する補題の名前に ``sets`` という単語が登場するものが散見される理由です．
...
OMIT. -/
/- TEXT:
これらの例から，関数 ``s : X → Y`` がある ``F : Filter X`` に沿って ``G : Filter Y`` に収束するとはどういうことかを以下のように定義できます．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
押し出しによる構成は写像を使用して写像の入力から出力にフィルタを押し出します．また *引き戻し(pullback)*  ``Filter.comap`` という反対方向の演算もあります．これは集合上の逆像に対する操作を一般化したものです．任意の写像 ``f`` に対して， ``Filter.map f`` と ``Filter.comap f`` は各 ``F`` と ``G`` に対して以下を満たす *ガロア接続* として知られるものを形成します．
...
OMIT. -/
/- TEXT:
フィルタの定義を ``Eventually`` の観点から復習することは有益です． ``X`` 上の任意の述語 ``P`` と ``Q`` に対して ``F : Filter X`` が与えられたとします，
TEXT. -/
/- OMIT:
