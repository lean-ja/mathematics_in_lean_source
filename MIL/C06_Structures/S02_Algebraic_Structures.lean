...
OMIT. -/
/- TEXT:
#. *半順序集合* は集合 :math:`P` と推移律と反対称律を持った :math:`P` 上の二項演算子 :math:`\le` からなります。
...
OMIT. -/
/- TEXT:
#. *群* は集合 :math:`G` に結合的な二項演算子と単位元 :math:`1` 、 :math:`G` の各 :math:`g` の逆元を返す関数 :math:`g \mapsto g^{-1}` を加えたものから成ります。群が *アーベル* もしくは *可換* であるとは二項演算子が可換であることを指します。
...
OMIT. -/
/- TEXT:
#. *環* は（加法的と言われる）アーベル群 :math:`(R, +, 0, x \mapsto -x)` に加法上に分配される結合的な乗法の演算子 :math:`\cdot` と単位元 :math:`1` を加えたものから成ります。環が *可換* であるとは乗法が可換であることを指します。
...
OMIT. -/
/- TEXT:
#. *順序環* :math:`(R, +, 0, -, \cdot, 1, \le)` は環に、 :math:`a + c \le b + c` が :math:`R` のすべての元 :math:`a` 、 :math:`b` 、 :math:`c` に対して成り立ち、また :math:`R` のすべての元 :math:`a` と :math:`b` が :math:`0 \le a` と :math:`0 \le b` を満たすときに :math:`0 \le a b` を成り立たせるような半順序を備えたものです。
...
OMIT. -/
/- TEXT:
#. *距離空間* は集合 :math:`X` と以下を満たすような関数 :math:`d : X \times X \to \mathbb{R}` から構成されます。
...
OMIT. -/
/- TEXT:
#. *位相空間* は集合 :math:`X` と、 :math:`X` の *開集合* と呼ばれる以下を満たすような :math:`X` の部分集合のあつまり :math:`\mathcal T` から構成されます。
...
OMIT. -/
/- TEXT:
これらの例では、各構造の元は *台集合* と呼ばれる集合に属し、この集合でその構造全体のことを指す場合もあります。例えば「 :math:`G` が群である」と言ってから「 :math:`g \in G` 」と言う時、私達は :math:`G` をそれぞれ構造と台集合の意味として使っています。すべての代数構造がこのように1つの台集合に関連付けられるわけではありません。例えば *2部グラフ* には *ガロア接続* のように2つの集合間の関係がふくまれます。 *圏* も一般に *対象* と *射* と呼ばれる2つの集合を中心として構成されます。
...
OMIT. -/
/- TEXT:
第三に、構造が他の構造から定義と定理、表記法を様々な方法で継承できるという事実に対処出来る必要があります。構造によっては公理を追加することで他の構造を拡張するものもあります。可換環は環でもあるので、環で成り立つ定義は可換環でも成り立ち、環で成り立つ定理は可換環でも成り立ちます。構造によっては乗法を追加することで他の構造を拡張することもあります。例えば環の加法についての部分は加法群です。環の構造はこの加法的な部分に乗法とその単位元、およびそれらを支配・関連する公理を追加します。ある構造を別の構造で定義出来ることもあります。任意の距離空間にはそれに関連する正準位相である *距離空間位相*  があり、任意の線形順序に関連付けることができる様々な位相があります。
...
OMIT. -/
/- TEXT:
最後に、数を定義するために関数や演算を使うのと同じように、構造を定義するために関数や演算を使うことができるということを覚えておくことが重要です。群の積と累乗もまた群です。すべての :math:`n` に対して、 :math:`n` を法として合同な整数は環を形成し、すべての :math:`k > 0` に対して、その環に係数を持つ多項式の :math:`k \times k` 行列もまた環を形成します。このように、構造を使って計算することは、その要素を使って計算するのと同じくらい簡単です。このことは代数的構造が数学に置いて対象のあつまりのためのコンテナとしてと、またそれ自身の対象としてとの二重の生活を送っていることを意味します。証明支援系はこの二重の役割に対応しなければなりません。
...
OMIT. -/
/- TEXT:
最初の素材はわざわざ言うほどではないほど明白なものです:形式的に言えば、代数構造は :numref:`section_structures` の意味での構造です。代数構造はいくつかの公理的な仮説を満たすデータのたばの仕様であり、これこそが ``structure`` コマンドが対応するように設計されていることを :numref:`section_structures` で見ました。まさにこれ以上無いほどのお似合いの関係です！
...
OMIT. -/
/- TEXT:
ここで型 ``α`` は ``group₁`` の定義における *パラメータ* であることをに注意してください。したがってオブジェクト ``struc : Group₁ α`` は ``α`` 上の群構造であると考えると良いでしょう。他の群の公理から ``mul_left_inv`` と対になる ``mul_right_inv`` が成り立つことは :numref:`proving_identities_in_algebraic_structures` で見たため、定義に追加する必要はありません。
...
OMIT. -/
/- TEXT:
理由は後ほど明らかに成りますが、型 ``α`` と構造体 ``Group α`` を分けておいたほうが便利なことが多いです。このような2つの対象を合わせて *部分的に束ねられた構造体* と呼びます。これは全てではないですが、多くの部分の成分を一つの構造に組み込む表現になっているからです。Mathlibでは群の台の型として使用する場合、型名としては大文字のアルファベット ``G`` を使用するのが一般的です。
...
OMIT. -/
/- TEXT:
さて、群を、つまり ``Group₁`` 型の元を構成してみましょう。型 ``α`` と``β`` の任意のペアに対して、Mathlibは ``α`` と ``β`` の間の *同値* を示す型 ``Equiv α β`` を定義しています。また、Mathlibはこの型に対して ``α ≃ β`` という示唆的な記法を定義しています。この型の要素 ``f : α ≃ β`` は ``α`` と ``β`` の間の双射で、4つの要素で表現されます: ``α`` から ``β`` への関数 ``f.toFun`` 、 ``β`` から ``α`` への逆関数 ``f.invFun`` 、そしてこれらの関数が互いに逆であることを示す2つの性質です。
EXAMPLES: -/
section
-- QUOTE:
variable (α β γ : Type*)
...
OMIT. -/
/- TEXT:
最後の3つの構文の特徴的な命名に注目してください。恒等関数 ``Equiv.refl`` 、逆演算 ``Equiv.symm`` 、合成演算 ``Equiv.trans`` は双射という性質が同値関係であることの明示的な証拠であると考えられます。
...
OMIT. -/
/- TEXT:
また、 ``f.tans g`` は逆順に前方の関数を合成する必要があることにも注意してください。Mathlibは ``Equiv α β`` から関数型 ``α → β`` への *型の強制* を宣言しているため、 ``.toFun`` を書くのを省略して、代わりにLeanに入れてもらうことができます。
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
この ``Equiv.Perm α`` が同値の合成で群を形成することは明らかでしょう。 ``mul f g`` は ``g.trans f`` と等しく、これで導かれる関数は ``f ∘ g`` です。言い換えれば、乗法は通常、双射の合成と考えられているもののことです。ここでこの群を定義します:
EXAMPLES: -/
-- QUOTE:
...
SOLUTIONS: -/
  ...
  zero_add : ∀ x : α, add x zero = x  -- TODO: add zero x = xでは？
...
OMIT. -/
/- TEXT:
実際、Mathlibはすでに ``Equiv.Perm α`` に対して一般的な群の表記、定義、定理を使うことができるように設定しています。
EXAMPLES: -/
section
-- QUOTE:
...
OMIT. -/
/- TEXT:
問題は、Leanは私達が入力する式の中にある情報を使って関連する表記と暗黙の群構造を *見つける* ことが出来る必要がある、ということです。例えば、 ``ℝ`` 型の ``x`` と ``y`` で ``x + y`` という式を書く時、Leanは ``+`` という起動を実数上の加算関数として解釈する必要があります。また、可換環の定義や定理をすべて利用できるように、 ``ℝ`` 型を可換環のインスタンスとして認識する必要があります。別の例として、Leanにおいて連続性は2つの位相空間の関係として定義されます。 ``f : ℝ → ℂ`` があり、 ``Continuous f`` と書く時、Leanは ``ℝ`` と ``ℂ`` 上の関連する位相を見つけなければなりません。
...
OMIT. -/
/- TEXT:
#. *論理* 。任意の群で解釈されるべき定義は、群の型と群構造を引数として取ります。同様に、任意の群の要素に関する定理は、群の型と群構造に対する普遍量化子から始まります。
...
OMIT. -/
/- TEXT:
#. *暗黙的な引数* 。型と構造の引数は一般的には暗黙的なままにしておき、私達が書いたり、Leanの情報ウィンドウで見たりする必要がないようにします。Leanがこっそり情報を埋めてくれます。
...
OMIT. -/
/- TEXT:
#. *型クラスの推論* 。もしくは *クラス推論* とも呼ばれるこのメカニズムは、Leanが後で使用する情報を登録できるシンプルで強力な仕組みです。Leanが定義、定理、表記法の一部に対する暗黙の引数を埋めるために呼び出された時、この登録された情報を利用することができます。
...
OMIT. -/
/- TEXT:
Leanが必要なときに見つけられるように情報を登録するこのシンプルな仕組みは驚くほど便利です。ここで一つの方法を紹介しましょう。Leanの基礎では、データ型 ``α`` は空であることがあります。しかし、実用にあたって多くの場合には型に少なくとも1つの要素があることを知っておくと便利です。例えば、リストの最初のと嘘を返す関数 ``List.headl`` はリストがからの場合にデフォルト値を返すことができます。これを実現するために、Leanのライブラリでは ``Inhabited α`` クラスを定義しています。 ``Point`` 型がこのインスタンスであることを示すことができます:
EXAMPLES: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
ここで挙げた例は危険です。なぜなら、Leanのライブラリには ``Group (Equiv.Perm α)`` のインスタンスもあり、乗算は任意の群で定義されているからです。そのため、どちらのインスタンスが見つかるかは曖昧です。実際、Leanは明示的に優先順位を指定しない限り、より新しい宣言を優先します。また、ある構造体が別の構造体のインスタンスであることをLeanに伝えるには、 ``extends`` キーワードを使う方法もあります。これは例えばMathlibにおいてすべての可換環が環であることの指定に使われています。より詳しい情報は *Theorem Proving in Lean* のクラス推論に関する節 `section on class inference <https://leanprover.github.io/theorem_proving_in_lean4/type_classes.html#managing-type-class-inference>`_ にあります。
...
OMIT. -/
/- TEXT:
クラス推論は微妙なものであり、使用する際には注意しなければなりません。というのも私達が入力した式に対する解釈を目に見えない形で支配する自動化を設定しているからです。しかし、賢く使えばクラス推論は強力なツールとなります。Leanで代数的推論を可能にしてくれます。
TEXT. -/
